...
> ✅ **Done** — code + docs: `9c28d60` (render/write leaks closed, aggregates count only real values, plans/22 corrected + rule 5 added). **Data null run on prod 2026-07-17**: `DELETE 2` orphan test rows + `UPDATE 2` (5 perf-less blocks nulled across 2 of Arthur's rows), post-check confirmed — `mqvmfv…` block 1 (`RX/7/rounds=500`) kept, all `perf`/drift rows untouched, Paulo's real `RX @ 7` block correctly left alone.
>
