-- submit_pr / clear_pr — RPCs used by me.html's PR log sheet (src/public/me/Me.jsx).
-- Verbatim from the repo-root supabase-rpcs.sql (historical dashboard SQL, one level
-- above cone/ — missed in 0001_init.sql's initial pass since it wasn't inside cone/),
-- with search_path pinned on both SECURITY DEFINER functions to match 0001's hardening.
...
BEGIN
  -- Load row
  SELECT value INTO v_goals FROM public.goals_data WHERE id = 1;
  ...
  UPDATE public.goals_data SET value = v_goals WHERE id = 1;
  IF NOT FOUND THEN
    INSERT INTO public.goals_data(id, value) VALUES(1, v_goals);
  END IF;
END;
$$;
