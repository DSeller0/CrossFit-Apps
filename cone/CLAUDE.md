...
**`AgendaView.jsx`** — rewritten by #59 · C5·a, see its own section below. `App.jsx` lazy-loads
`AgendaView` straight from `publicador/AgendaView`, not through `Publicador.jsx`, so opening
Agenda no longer drags in the export/PDF graph. ⚠️ **`Publicador.jsx` + the two export-view files
are still `React.createElement`, not JSX** — kept that way on purpose so C5·b's rewrite is the
first JSX pass over that markup, not a second one. The Agenda half is JSX as of C5·a.
