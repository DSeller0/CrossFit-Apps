...
- `blobTables.js` — `BLOB_TABLES` (the 8-table fetch-order array; positional, don't reorder without updating every destructuring). **Since #52 retired `athletes.html`, `Leaderboard.jsx` is its only consumer** — and it still carries the dead `lb_colors` slot, so fold the cleanup into #60/#43 rather than churning it. `mapResultRow` (the `results_v2` snake→camel mapper) is the widely-used half: `Me.jsx` was hand-writing a fourth copy of it until #52, and `src/utils/resultMappers.js`'s SPA-side `rowToResult` aliases it since #70.
