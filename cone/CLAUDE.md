...
- **Component gallery:** `gallery.html` (repo root) + `cone/src/public/gallery/` (`main.jsx`, `Gallery.jsx`) — theme switcher + width toggle rendering the real components in every state from mock fixtures; `GROUPS` holds 34 items across Shared/Results/Leaderboard/Me/Schedule/**Index** (the Index group = the #53 rail: WeekStrip/TodayRanking/BoxNotice), picked from a sidebar. **Dev-only:** NOT in `vite.public.config.js` `input`, so `npm run dev:public` serves it at `/CrossFit-Apps/gallery.html` but it is never built/deployed. Grows page-by-page as components are extracted (#17).
