...
- Tests: `npm test` (**779 tests across 23 files**, re-measured 2026-08-06 — added `stateBackup` (`components/tabs/config/`, plans/69) and `billing` (`components/tabs/publicador/`, plans/71): wod, week, sessions, goals, registry, boxScope, exerciseGroups, resultEntry, theme (`public/lib/`) · entries (`public/`) · meHelpers (`public/me/`) · scheduleHelpers (`public/schedule/`) · pix, resultMappers, storage, config (`utils/`) · blockModel, textFormat (`criador/`) · exerciciosHelpers, resultadosHelpers, stateBackup, billing (`components/tabs/`) · useClassTracking (`hooks/`))
