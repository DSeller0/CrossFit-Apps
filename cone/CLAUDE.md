...
**Publicador (#25 · plans/39 → #59 · C5·b1 · plans/82, shipped 2026-09-04)** —
`src/components/tabs/Publicador.jsx` is the `SchedulePublisher` container, **574 lines** (was
2224 pre-redesign), over `src/components/tabs/publicador/`: `exportHelpers.js` (pure formatters),
`exportViews.jsx` (`DailyExportView`/`WeeklyExportView`/`WeeklyCalendarExportView`/
`CalendarExportView`), `mobileExportViews.jsx` (`MobileEaglesExportView`/`MobileMegaManExportView`/
`MobileWeeklyExportView`), `events.jsx` (`EventFormInner` + `ReportModal`), `pixQr.js` (#162/plans/78
...
Format names + the `--a-*` contract, precedence and every acceptance bar: [plans/82](./docs/plans/82-c5b1-publicador-shell-e-cores.md). **Next: C5·b2** ([plans/83](./docs/plans/83-c5b2-publicador-renderer.md)) makes the artefacts themselves parametric (Layout · Blocos · Títulos · fit) —
b1 deliberately left those alone.
