...
- `SegBar.jsx` — the segmented HP bar (#52). One primitive for me.html's goal bars + milestone ticks, its PR mini-bars, and (plans/22) the Desenvolvimento stats card. The three copies it replaced each faked their segments with a `repeating-linear-gradient` that had the **dark theme's background baked in as a literal `rgba(13,11,9,.65)`**, so both light themes painted dark bands across the bar. Dividers are real elements in `var(--bg)`; ticks sit outside the track's `overflow:hidden` so a milestone at 0%/100% isn't sliced in half.
...
- `blobTables.js` — `BLOB_TABLES` (the 8-table fetch-order array; positional, don't reorder without updating every destructuring). **Since #52 retired `athletes.html`, `Leaderboard.jsx` is its only consumer** — and it still carries the dead `lb_colors` slot, so fold the cleanup into #60/#43 rather than churning it. `mapResultRow` (the `results_v2` snake→camel mapper) is the widely-used half: `Me.jsx` was hand-writing a fourth copy of it until #52.
