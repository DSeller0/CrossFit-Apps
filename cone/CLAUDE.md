...
`src/components/tabs/Publicador.jsx` is the `SchedulePublisher` shell only (**2224 raw lines** — the "~660" this note carried until 2026-08-29 was a PRE-reformat figure, see BACKLOG.md's unit-discontinuity warning); the rest
lives in `src/components/tabs/publicador/`: `exportHelpers.js` (pure formatters — the dictation
mic button it used to also hold was dead code, deleted #59 Phase 0), `exportViews.jsx` (`DailyExportView`/`WeeklyExportView`/
...
- ⚠️ **`index.css` keeps the 7 `rp-*` rules AgendaView shares** (`rp-sticktop` · `rp-month-nav` ·
