...
async function main() {
  ...
  const { data: src, error: srcErr } = await supabase
    ...
    .maybeSingle()
  if (srcErr) {
    console.error('Failed to load results blob:', srcErr.message)
    process.exit(1)
  }
  ...
  rows.forEach(r =>
    console.log(`  ${r.id}  athlete:${r.athlete_id}  session:${r.session_id}  date:${r.date}`),
  )

  const { error: upsertErr } = await supabase.from('results_v2').upsert(rows, { onConflict: 'id' })
