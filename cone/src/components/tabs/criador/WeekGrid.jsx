...
export function WeekGrid({
  ...
  const weekSessions = weekDates.flatMap(d =>
    (sessions[toISO(d)] || [])
      .filter(boxFilter)
      .map(sess => ({ date: d, dateKey: toISO(d), sess })),
  )
  ...
  const copyWeek = () =>
    copy(
      ...
      weekSessions
        .map(
          ({ date, sess }) =>
            `${DAY_PT[date.getDay()]} ${date.getDate()} — ${sessName(sess, toISO(date))}\n\n${serializeSession(sess)}`,
        )
        .join('\n\n────────────\n\n'),
    )
      ...
      <div className={cr.stickyHead} ref={gridRef}>
        <div className={cr.weekBar}>
          ...
          <span className={s.modeSeg} role="group" aria-label="Modo da grade">
            <button
              type="button"
              className={!isText ? s.on : ''}
              aria-pressed={!isText}
              onClick={() => setGridMode('grade')}
              title="Ver como lista de exercícios"
            >
              ▤ Grade
            </button>
            <button
          ...
          {isText && weekSessions.length > 0 && (
            <Button size="sm" onClick={copyWeek} title="Copiar a semana inteira como texto">
              <i className={`ti ${copied === 'week' ? 'ti-check' : 'ti-copy'}`} />{' '}
              {copied === 'week' ? 'Copiado' : 'Copiar semana'}
            </Button>
          )}
          {onImport && (
          ...
          <Button
            ...
            aria-label={
              weekGridCollapsed ? 'Expandir grade da semana' : 'Minimizar grade da semana'
            }
            ...
            if (!list.length) {
              return (
                ...
                >
                  ...
                  <span className={s.mDay}>
                    {DAY_PT[di]} {date.getDate()}
                  </span>
            ...
            return list.map(sess => {
              ...
              return (
                <div key={sess.id}>
                  <button
                    type="button"
                    className={`${s.mRow} ${open ? s.mRowOpen : ''}`}
                    aria-expanded={open}
                    onClick={() => toggleOpen(sess.id)}
                  >
                    <span className={s.mChev}>
                    ...
                    <span className={s.mCount}>
                      {(sess.blocks || []).length} bloco
                      {(sess.blocks || []).length !== 1 ? 's' : ''}
                    </span>
                  </button>
                  {open && (
                    <div className={s.mBody}>
                      ...
                      <div className={s.mActions}>
                        ...
                        <Button
                          size="sm"
                          iconOnly
                          aria-label={`Editar ${sessName(sess, dateKey)}`}
                          title="Editar"
              ...
              return (
                <div key={dateKey} className="wg-col">
                  ...
                  {list.map(sess => (
                    ...
                    <div
                      key={sess.id}
                      className={`wg-sc${sess.id === highlightedSessionId ? ' wg-saved' : ''}`}
                      draggable
                      role="button"
                      tabIndex={0}
                      aria-label={`Abrir ${sessName(sess, dateKey)}`}
                      style={{
                        outline: editing?.id === sess.id ? '2px solid var(--accent)' : 'none',
                        outlineOffset: 1,
                      }}
                      onClick={() => startEdit(sess, dateKey)}
                      onKeyDown={e => {
                        if (e.key === 'Enter' || e.key === ' ') {
                          e.preventDefault()
                          startEdit(sess, dateKey)
                        }
                      }}
                      onDragStart={e => {
                        e.dataTransfer.setData('sess-id', sess.id)
                        e.dataTransfer.setData('sess-date', dateKey)
                      ...
                      onDrop={e => {
                        ...
                        moveSession(
                          e.dataTransfer.getData('sess-id'),
                          e.dataTransfer.getData('sess-date'),
                          dateKey,
                          sess.id,
                        )
                      }}
                      ...
                      <div className={cr.cardHd}>
                        ...
                        <span className={cr.cardHdBtns}>
                          ...
                          <Button
                            ...
                            iconOnly
                            variant="ghost"
                            className={cr.dangerGhost}
                            aria-label={`Remover ${sessName(sess, dateKey)}`}
                            onClick={e => {
                              e.stopPropagation()
                              onDelete(dateKey, sess.id)
                            }}
                          >
                            <i className="ti ti-x" />
                          </Button>
