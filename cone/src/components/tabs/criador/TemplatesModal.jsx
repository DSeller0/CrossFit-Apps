...
export function TemplatesModal({ templates, onClose, onApply, onDelete, onRecurring }) {
  return (
    <div className={s.scrim} onClick={onClose}>
      <div className={`${s.modal} ${s.modalSm}`} role="dialog" aria-modal="true" aria-labelledby="cr-tpl-title"
        onClick={e => e.stopPropagation()}>
        <div className={s.modalHd}>
          <span id="cr-tpl-title" className={s.modalTitle}>Templates</span>
        ...
        <div className={s.modalBody}>
          {templates.length === 0
            ...
            : <div className={s.tplList}>
                {templates.map(tpl => (
                  <div key={tpl.id} className={s.tplCard} role="button" tabIndex={0}
                    ...
                    <div className={s.tplBody}>
                      ...
                      <div className={s.tplSub}>{tpl.blocks.length} bloco{tpl.blocks.length !== 1 ? 's' : ''}</div>
                      {tpl.blocks.length > 0 && (() => {
                    ...
                    <Button size="xs" iconOnly aria-label={`Sessões recorrentes de ${tpl.name}`}
                      title="Sessões recorrentes"
                      onClick={e => { e.stopPropagation(); onRecurring(tpl); }}>
                      <i className="ti ti-repeat" />
                    </Button>
                    <Button size="xs" iconOnly variant="destructive" aria-label={`Excluir template ${tpl.name}`}
