      ...
      {open && suggestions.length > 0 && dropRect && (
        ...
        >
          {suggestions.map((sug, i) => (
            <div
              key={i}
              className={`ex-suggestion ${s.comboItem}`}
              tabIndex={0}
              role="option"
              onMouseDown={e => {
                e.preventDefault()
              ...
              onKeyDown={e => {
                ...
                if (e.key === 'ArrowUp') {
                  ...
                  prev ? prev.focus() : ref.current?.querySelector('input')?.focus()
                }
                if (e.key === 'Escape') {
                  setOpen(false)
            ...
            >
              ...
              <span
                className={s.comboDot}
                style={{ background: ECOL[sug.blockType]?.text || 'var(--muted)' }}
              />
              {sug.name}
            </div>
          ))}
