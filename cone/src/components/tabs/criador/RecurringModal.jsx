...
export function RecurringModal({
  ...
  return (
    <div className={s.scrim} onClick={onClose}>
      <div
        className={`${s.modal} ${s.modalMd}`}
        role="dialog"
        aria-modal="true"
        aria-labelledby="cr-recur-title"
        ...
        <div className={s.modalBody}>
            ...
            <div className={s.recurDays} role="group" aria-label="Dias da semana">
              {[
                ...
                <button
                  key={day}
                  type="button"
                  aria-pressed={recurDays.has(day)}
          ...
          {recurPreviewDates.length > 0 && (
            <div className={s.recurPreview}>
              <div className={s.recurPreviewHd}>
                {recurPreviewDates.length} sessão{recurPreviewDates.length !== 1 ? 'ões' : ''} a
                criar:
              </div>
              <div className={s.recurChips}>
                {recurPreviewDates.map(d => (
                  <span key={d} className={s.recurChip}>
                    {d}
                  </span>
                ))}
              </div>
          ...
          {recurPreviewDates.length === 0 && recurDays.size > 0 && (
            <div className={s.hintCenter}>Nenhuma data no período.</div>
          )}
        </div>
        ...
        <div className={s.modalFoot}>
          {recurDone != null ? (
            <div className={s.doneMsg} role="status">
              <i className="ti ti-check" aria-hidden="true" /> {recurDone} sessão
              {recurDone !== 1 ? 'ões' : ''} criada{recurDone !== 1 ? 's' : ''}!
            </div>
          ) : (
            <Button variant="primary" full disabled={!recurPreviewDates.length} onClick={onApply}>
              <i className="ti ti-calendar-plus" aria-hidden="true" /> Criar{' '}
              {recurPreviewDates.length} sessão{recurPreviewDates.length !== 1 ? 'ões' : ''}
            </Button>
          )}
        </div>
      </div>
