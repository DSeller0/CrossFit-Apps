...
import {
  materializeEx, normalizeCardioEx, normalizeLegacyCardio,
  blockSummary, stationsCapStr, loadBadgeStr,
} from './blockModel.js'

describe('materializeEx', () => {
  const reg = { 'Força': [{ name: 'Back Squat', defaults: { sets: '5', reps: '5', intensity: { mode: 'pct', pct: 80 } } }] }

  test('fills empty sets/reps from registry defaults', () => {
    const out = materializeEx({ name: 'Back Squat', sets: '', reps: '', dist: '', intensity: null }, reg)
    expect(out.sets).toBe('5')
    expect(out.reps).toBe('5')
  })

  test('does not overwrite already-filled sets/reps', () => {
    const out = materializeEx({ name: 'Back Squat', sets: '3', reps: '10', dist: '', intensity: null }, reg)
  ...
  test('dist and reps are mutually exclusive: existing reps blocks a dist default', () => {
    ...
    const out = materializeEx({ name: 'Run', sets: '', reps: '10', dist: '', intensity: null }, distReg)
  ...
  test('dist and reps are mutually exclusive: existing dist blocks a reps default', () => {
    ...
    const out = materializeEx({ name: 'Foo', sets: '', reps: '', dist: '50', intensity: null }, repsReg)
  ...
  test('intensityDefaultDismissed suppresses the registry intensity default and is stripped from the saved shape', () => {
    const out = materializeEx({ name: 'Back Squat', sets: '5', reps: '5', dist: '', intensity: null, intensityDefaultDismissed: true }, reg)
  ...
  test('without dismissal, registry intensity default applies', () => {
    const out = materializeEx({ name: 'Back Squat', sets: '5', reps: '5', dist: '', intensity: null }, reg)
    expect(out.intensity).toEqual({ mode: 'pct', pct: 80 })
  })

  test('a complex exercise never takes registry defaults', () => {
    const out = materializeEx({ name: 'Back Squat', isComplex: true, sets: '', reps: '', dist: '', intensity: null, complexMovements: [] }, reg)
...
describe('normalizeCardioEx', () => {
  test('legacy cardio mode → dist/distUnit siblings, intensity cleared', () => {
    const out = normalizeCardioEx({ name: 'Run', intensity: { mode: 'cardio', cardioVal: '400', cardioUnit: 'm' } })
...
describe('normalizeLegacyCardio', () => {
  test('Estações block walks stations[].exercises[]', () => {
    const blocks = [{ type: 'Estações', stations: [{ exercises: [{ name: 'Run', intensity: { mode: 'cardio', cardioVal: '200', cardioUnit: 'm' } }] }] }]
  ...
  test('standard block walks exercises[] directly', () => {
    const blocks = [{ type: 'For Time', exercises: [{ name: 'Row', intensity: { mode: 'cardio', cardioVal: '500', cardioUnit: 'm' } }] }]
    const out = normalizeLegacyCardio(blocks)
...
describe('blockSummary', () => {
  test('benchmarkRef short-circuits everything else', () => {
    expect(blockSummary({ benchmarkRef: 'Fran', type: 'For Time', duration: '20', rounds: '5' })).toBe('Fran')
  })

  test('Estações: groups/rests/repeat/cap', () => {
    const block = {
      type: 'Estações', stationRepeat: 2,
      stations: [{ duration: '1:00', isRest: false }, { duration: '0:30', isRest: true }],
    }
    expect(blockSummary(block)).toBe("Cap 3' · 1 grupos · 1 descanso · ×2")
  })

  test('standard block: duration/rounds/movement count', () => {
    const block = { type: 'For Time', duration: '20', rounds: '5', exercises: [{ name: 'Run' }, { name: '' }, { name: 'Row' }] }
...
describe('stationsCapStr', () => {
  ...
  test('restBetweenCycles counts only between cycles, not after the last one', () => {
    const block = { type: 'Estações', stationRepeat: 2, restBetweenCycles: '1:00', stations: [{ duration: '1:00' }] }
    expect(stationsCapStr(block)).toBe("Cap 3'")
  })
})
...
describe('loadBadgeStr', () => {
  ...
  test('progression mode with a load on the first step → load+unit', () => {
    expect(loadBadgeStr({ intensity: { mode: 'progression', steps: [{ load: '70', unit: '% do RM' }] } })).toBe('70%')
  })
