...
export function SessionMetaModal({ initial, athletes = [], boxLocs = [], isEdit = false, onCancel, onConfirm }) {
  ...
  const toggleAthlete = name => set({
    mainTraining: targets.includes(name) ? targets.filter(n => n !== name) : [...targets, name],
  });
  const toggleBox = id => set({
    locationIds: id === null ? [] : (boxIds.includes(id) ? boxIds.filter(x => x !== id) : [...boxIds, id]),
  });

  return (
    <div className={s.scrim} onClick={onCancel}>
      <div className={s.modal} role="dialog" aria-modal="true" aria-labelledby="cr-meta-title"
        ...
        <div className={s.modalBody}>
          <div className={s.row2}>
            <Input label="Data" type="date" value={d.date || ''}
              onChange={e => set({ date: e.target.value })} />
            <Input label="Nome da sessão" placeholder="ex: Semana 3 · D1 · Força Lower"
              value={d.sessionName || ''} onChange={e => set({ sessionName: e.target.value })} />
          </div>

          <div>
            <span className={s.lbl}>Para quem</span>
            <button type="button" className={`${s.athBtn}${targets.length ? '' : ' ' + s.athBtnEmpty}`}
              aria-expanded={showAth} onClick={() => setShowAth(v => !v)}>
              <i className="ti ti-users" aria-hidden="true" />
              {targets.length === 0
                ? 'Nenhum atleta — clique para selecionar'
                : targets.map(n => <span key={n} className={s.athChip}>{n}</span>)}
            </button>
            {showAth && (
              <div className={s.athList}>
                {athletes.length === 0 && <span className={s.athLevel}>Nenhum atleta cadastrado.</span>}
                {athletes.map(a => {
                  const on = targets.includes(a.name);
                  return (
                    <label key={a.id} className={`${s.athRow}${on ? ' ' + s.athRowOn : ''}`}>
                      <input type="checkbox" checked={on} onChange={() => toggleAthlete(a.name)}
            ...
            <div>
              ...
              <div className={s.pillRow}>
                {[{ id: null, name: 'Sem box' }, ...boxLocs].map(b => {
                  ...
                  return (
                    <button key={b.id || 'none'} type="button" aria-pressed={on}
