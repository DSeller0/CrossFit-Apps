...
export const normalizeCardioEx = ex =>
  ex.intensity?.mode === 'cardio'
    ? {
        ...ex,
        dist: ex.intensity.cardioVal || ex.dist || '',
        distUnit: ex.intensity.cardioUnit || 'm',
        intensity: null,
      }
    : ex
export const normalizeLegacyCardio = blocks =>
  (blocks || []).map(bl =>
    bl.type === 'Estações'
      ? {
          ...bl,
          stations: (bl.stations || []).map(st => ({
            ...st,
            exercises: (st.exercises || []).map(normalizeCardioEx),
          })),
        }
...
export const materializeEx = (ex, reg) => {
  ...
  const distEmpty = !String(out.dist || '').trim(),
    repsEmpty = !String(out.reps || '').trim()
  if (regDefaults.dist && distEmpty && repsEmpty) {
    out.dist = regDefaults.dist
    out.distUnit = regDefaults.distUnit || out.distUnit || 'm'
  ...
  if (regDefaults.intensity && !hasIntensity && !intensityDefaultDismissed)
    out.intensity = regDefaults.intensity
  return out
}
export const materializeBlocks = (blocks, reg) =>
  (blocks || []).map(bl =>
    bl.type === 'Estações'
      ? {
          ...bl,
          stations: (bl.stations || []).map(st => ({
            ...st,
            exercises: (st.exercises || []).map(ex => materializeEx(ex, reg)),
          })),
        }
...
export const TYPE_CONFIG = {
  ...
  HIIT: {
    icon: 'ti-bolt',
    color: '#e05848',
    desc: 'Alta intensidade intervalado',
    showDuration: true,
    showRounds: true,
    durationLabel: 'Intervalo (s)',
  },
  MetCon: {
    icon: 'ti-flame',
    color: '#c84040',
    desc: 'Condicionamento misto',
    showDuration: true,
    showRounds: true,
  ...
  'For Time': {
    icon: 'ti-clock',
    color: '#c86828',
    desc: 'Contra o relógio',
    showDuration: true,
    showRounds: true,
    durationLabel: 'Time cap (min)',
  },
  AMRAP: {
    icon: 'ti-refresh',
    color: '#e09830',
    desc: 'Máx rounds em tempo fixo',
    showDuration: true,
    showRounds: true,
    durationLabel: 'Duração (min)',
  },
  Estações: {
    icon: 'ti-map-pin',
    color: '#c8a030',
    desc: 'Treino por grupos / estações',
    showDuration: false,
    showRounds: false,
    durationLabel: 'Duração (min)',
    isStations: true,
  ...
  Mobilidade: {
    icon: 'ti-leaf',
    color: '#30a868',
    desc: 'Mobilidade e flexibilidade',
    showDuration: true,
    showRounds: true,
    durationLabel: 'Duração (min)',
  },
  ...
  Benchmark: {
    icon: 'ti-trophy',
    color: '#d8a840',
    desc: 'Benchmark WOD (Girls, Heroes, Custom)',
    showDuration: true,
    showRounds: true,
    durationLabel: 'Time cap (min)',
  },
}
export const DEFAULT_TYPE_CFG = {
  icon: 'ti-edit',
  color: '#888',
  desc: 'Bloco livre',
  showDuration: true,
...
export function stationsCapStr(block) {
  ...
  const parse = v => {
    ...
    return p.length >= 2 ? (+p[0] || 0) * 60 + (+p[1] || 0) : (+p[0] || 0) * 60
  }
...
export function blockSummary(block) {
  ...
  if (block.type === 'Estações') {
    ...
    return [cap, groups && `${groups} grupos`, rests && `${rests} descanso`, rep]
      .filter(Boolean)
      .join(' · ')
  }
