...
export function WeekImportModal({ weekDates, weekLabel, sessions, boxFilter, boxLocs, selBox, onPrevWeek, onNextWeek, onCreate, onClose, initialText = '' }) {
  const [text, setText] = useState(initialText);
  const [locationIds, setLocationIds] = useState(() => (selBox === 'all' || selBox === 'none') ? [] : [selBox]);
  ...
  const rows = useMemo(() => days.map(d => {
    ...
    const exCount = d.blocks.reduce((n, b) => n + (b.exercises || []).filter(e => (e.name || '').trim() || e.isComplex).length, 0);
    const noType = d.warnings.filter(w => w.kind === 'type-unresolved').length;
    const noted = d.warnings.filter(w => w.kind === 'unparsed-line' || w.kind === 'orphan-load').length;
  ...
  const create = () => {
    onCreate(creatable.map(r => ({
      dateKey: r.dateKey,
      session: {
        id: uid(), date: r.dateKey, sessionName: r.sessionName || '',
        mainTraining: [], locationIds, public: isPublic, blocks: r.blocks,
      },
    })));
  };
    ...
    <div className={s.scrim} onClick={onClose}>
      <div className={s.modal} role="dialog" aria-label="Importar semana" onClick={e => e.stopPropagation()}>
        ...
        <div className={s.modalBody}>
          <label className={s.lbl} htmlFor="wk-import-ta">Cole o texto da semana</label>
          <textarea
            id="wk-import-ta"
            className={s.ta}
            style={{ minHeight: 130 }}
          ...
          {!text.trim() ? (
            ...
            <div className={s.prevFoot} style={{ borderTop: 0, marginTop: 0, paddingTop: 0 }}>
              <span className={s.warnRow}>⚠ Nenhum dia da semana reconhecido.</span><br />
              <span className={s.infoRow}>Comece cada dia com o nome dele: <b>SEGUNDA-FEIRA</b>.</span>
            </div>
          ) : (
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className={r.taken ? s.detSkip : undefined}>
                    <td className={s.detDay}>{DAY_PT[r.dayIndex]} {r.date ? r.date.getDate() : '—'}</td>
                    <td className={s.detCount}>{r.blocks.length} blocos · {r.exCount} ex</td>
                    <td className={s.detState}>
                      {r.taken ? <span className={s.infoRow}>já tem sessão — pular</span>
                        : r.noType ? <span className={s.warnRow}>⚠ {r.noType} tipo por definir</span>
                        : r.noted ? <span className={s.infoRow}>⚠ {r.noted} linha como nota</span>
                        : <span style={{ color: 'var(--green)' }}>✓</span>}
                      {r.sessionName && <span style={{ color: 'var(--gold)', marginLeft: 6 }}>&ldquo;{r.sessionName}&rdquo;</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {boxLocs.length > 0 && (<>
            ...
            <div className={s.pills}>
              {[{ id: null, name: 'Sem box' }, ...boxLocs].map(b => {
                ...
                return (
                  <button key={b.id || 'none'} type="button" className={`${s.pill} ${on ? s.pillOn : ''}`}
                    onClick={() => setLocationIds(cur => b.id === null ? [] : (cur.includes(b.id) ? cur.filter(x => x !== b.id) : [...cur, b.id]))}>
                    {b.id && <span className={s.dot} style={{ background: b.color || 'var(--muted)' }} />}
                    {b.name}
                  </button>
                );
              })}
          ...
          <div className={s.pills}>
            {[['Público', true], ['Oculto', false]].map(([label, val]) => (
              <button key={label} type="button" className={`${s.pill} ${isPublic === val ? s.pillOn : ''}`}
                onClick={() => setIsPublic(val)}>{label}</button>
            ))}
          </div>
        </div>
