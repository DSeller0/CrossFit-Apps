  ...
  return (
    <>
      {blocks.map(bl => {
        ...
        const meta = [label, bl.rounds && `${bl.rounds}×`, bl.duration && `CAP ${bl.duration}'`]
          .filter(Boolean)
          .join(' · ')
        return (
          <div key={bl.id} className={s.cardBlk}>
            ...
            {bl.type === 'Estações' ? (
              <div className={s.prevMeta}>
                {(bl.stations || []).filter(st => !st.isRest).length} grupos
              </div>
            ) : (
