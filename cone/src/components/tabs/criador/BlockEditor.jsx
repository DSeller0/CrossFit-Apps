...
export function BlockEditor({
  ...
  const handleExUpdate = (exOrNull, fromIdx, toIdx) => {
    if (exOrNull === null) {
      ...
      onUpdate({
        ...block,
        exercises: block.exercises.map(x => (x.id === exOrNull.id ? exOrNull : x)),
      })
    }
  }
  ...
  const saveCustomBenchmark = () => {
    ...
    saveSettings({
      ...settings,
      customBenchmarks: [...customs.filter(c => c.name !== bm.name), bm],
    })
    setBmSaveFlash(true)
  ...
  return (
    <div
      ...
      style={{
        outline: dragOverBlkIdx === blockIdx ? '2px solid var(--theme-accent)' : 'none',
        outlineOffset: 2,
        borderRadius: 'var(--radius-md)',
        transition: 'outline .1s, border-color .15s',
        borderColor:
          collapsed && changedFields?.size
      ...
      onDragLeave={e => {
        if (!e.currentTarget.contains(e.relatedTarget)) setDragOverBlkIdx?.(null)
      }}
      onDrop={e => {
        e.preventDefault()
      ...
      {!collapsed && (
        <div className="blk-body">
          {(() => {
                    ...
                    {(block.exercises || []).map((ex, i) => {
                      ...
                      return (
                        <div key={ex.id} className="bm-locked-row">
                          ...
                          <span className="bm-locked-row-name">
                            {vol ? `${vol} ${ex.name}` : ex.name}
                          </span>
                          {badge && <span className="ex-load-badge">{badge}</span>}
                        </div>
                      )
            ...
            const fch = (...fields) =>
              fields.some(f => changedFields?.has(f))
                ...
                <div className="blk-meta-row">
                 ...
                 migration with its own backlog row, not an input swap. */
                    <Input
                      ...
                      label={cfg.durationLabel || 'Duração (min)'}
                      type="number"
                      min={1}
                      inputMode="numeric"
                      placeholder="—"
                      ...
                      {(block.exercises || []).map((ex, ei) => (
                        ...
                        >
                          <ExerciseRow
                            ...
                            onToggleLadder={() =>
                              onUpdate({ ...block, ladderMode: !block.ladderMode })
                            }
