...
import { mfs, exLine, complexLine, buildProgressionLines, buildMobileSession } from './exportHelpers';
...
export function MobileEaglesExportView({ sessions, selectedDate, currentWeekDates, gymName, logoDataUrl, logoScale, fontScale, bgOverride, colors }) {
  const col = colors || {};
...
export function MobileMegaManExportView({ sessions, selectedDate, currentWeekDates, gymName, logoDataUrl, logoScale, fontScale, bgOverride, colors }) {
  const col = colors || {};
  const found = buildMobileSession(sessions, selectedDate, currentWeekDates);
  const f = fontScale || 1;
  const pad = Math.round(28 * f);
...
export function MobileWeeklyExportView({ sessions, gymName, logoDataUrl, logoScale, fontScale, weekDates, variant }) {
  const f = fontScale || 1;
  const ls = logoScale || 1;
