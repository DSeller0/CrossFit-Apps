...
export function RegistroView({
  ...
  const rpeColor = rpe => {
    ...
    const t = (rpe - 1) / 9
    return `rgb(${Math.round(t < 0.5 ? 2 * t * (224 - 96) + 96 : 224)},${Math.round(t < 0.5 ? 168 : 168 - 2 * (t - 0.5) * 88)},64)`
  }

  const resultSummary = r => {
    if (r.presence !== 'Presente') return r.presence
    const perfs = (r.blocks || []).flatMap(b =>
      b.perfTime ? [b.perfTime] : b.perfRounds ? [`${b.perfRounds}rds`] : [],
    )
    ...
    <div className="rp-p1">
      <div className="rp-sticktop">
        <div className="rp-month-nav">
          <button type="button" className="rp-nav-btn" onClick={prevMonth}>
            ‹
          </button>
          <span className="rp-month-label">
            {MONTH_PT[viewMonth]} {viewYear}
          </span>
          <button type="button" className="rp-nav-btn" onClick={nextMonth}>
            ›
          </button>
        </div>
      ...
      {weekDays.map(({ date, dk, daySessions: ds }) => {
        ...
        if (!inMonth)
          return (
            <div key={dk} className="rp-rest-day" style={{ opacity: 0.3 }}>
              {dayName} {dayNum}
            </div>
          )
        return (
          <div key={dk}>
            <div className="rp-day-hdr">
            ...
            {ds.length === 0 ? (
              ...
              ds.map(sess => {
                ...
                const logged = results.filter(
                  r => r.date === dk && (r.sessionId === sess.id || (!r.sessionId && !sess.id)),
                ).length
                const on = selKey === k
                return (
                  <div
                    key={k}
                    className={`rp-sess-card${on ? ' on' : ''}`}
                    onClick={() => {
                      setSelKey(k)
                      setSelAthlete(null)
                      setAddOpen(false)
                  ...
                  >
                    ...
                    <div className="rp-sess-sub">
                      <span style={{ color: logged > 0 ? '#4ac8c0' : '#554a3a' }}>
                        {logged}/{athletes.length} reg.
                      </span>
                      {(sess.blocks || [])
                        .filter(isWodBlock)
                        .slice(0, 2)
  ...
  const renderP2 = () => {
    ...
    const dateLabel = new Date(selDateKey + 'T12:00:00').toLocaleDateString('pt-BR', {
      weekday: 'long',
      day: '2-digit',
      month: 'long',
    })
    return (
      <div className="rp-p2">
        ...
        {loggedList.map(a => {
          const r = loggedAthMap[a.id],
            on = selAthlete?.id === a.id,
            del = p2Del === a.id
          return (
            <div
              key={a.id}
              className={`rp-ath-row${on ? ' on' : ''}`}
              onClick={() => {
                if (del) return
                setSelAthlete(a)
                setAddOpen(false)
            ...
            >
              ...
              {del ? (
                <div className="rp-del-inline" onClick={e => e.stopPropagation()}>
                  <span style={{ fontSize: 10, color: '#e05050', whiteSpace: 'nowrap' }}>
                    Excluir?
                  </span>
                  <button
                    type="button"
                    className="b bd bsm"
                    style={{ minWidth: 34, padding: '2px 6px', fontSize: 10 }}
                ...
                <button
                  type="button"
                  className="b bd bsm"
                  style={{ minWidth: 28, padding: '3px 7px', flexShrink: 0 }}
                  onClick={e => {
                    e.stopPropagation()
                    setP2Del(a.id)
                  }}
        ...
        {loggedList.length === 0 && (
          <div
            style={{ padding: '14px 12px', fontSize: 11, color: '#806850', textAlign: 'center' }}
          >
            ...
            {addOpen && (
              <div className="rp-add-dropdown">
                {unloggedList.map(a => (
                  ...
                  >
                    <div
                      className="rp-ath-dot"
                      style={{ background: a.color || '#e87820', width: 7, height: 7 }}
                    />
                    {a.name}
                  </div>
                ))}
      ...
      <div className="rp-p3">
        ...
        <div className="rp-sticktop">
          <div className="rp-p3-hdr">
            <div
              className="rp-ath-dot"
              style={{ background: selAthlete.color || '#e87820', width: 10, height: 10 }}
            />
            <div className="rp-p3-name">{selAthlete.name}</div>
            {selAthlete.level && (
              <span className={`level-badge ${LEVEL_CLS[selAthlete.level] || 'lv-ini'}`}>
                {selAthlete.level}
              </span>
            )}
            {saveFlash && (
        ...
        <div className="rp-p3-body">
            ...
            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
              {PRESENCE.map(p => (
                <button
                  ...
                  className="b bsm"
                  style={{
                    background: presence === p ? 'var(--theme-accent)' : 'transparent',
                    color: presence === p ? 'var(--theme-accent-text)' : '#806850',
                    borderColor: presence === p ? 'var(--theme-accent)' : '#2a231c',
                  }}
                ...
                {[1, 2, 3, 4, 5].map(n => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontSize: 13,
                      background: energyLevel === n ? 'var(--theme-accent)' : 'transparent',
                      color: energyLevel === n ? 'var(--theme-accent-text)' : '#806850',
                      borderColor: energyLevel === n ? 'var(--theme-accent)' : '#2a231c',
                    }}
                        ...
                        {exercises.map((ex, ei) => {
                          ...
                          return (
                            <div key={ei} className="rp-wod-ex">
                              {vol && (
                                <span style={{ color: '#d8a840', fontWeight: 700, flexShrink: 0 }}>
                                  {vol}
                                </span>
                              )}
                              <span>{ex.name}</span>
                    ...
                    <div className="rp-rpe-row">
                      ...
                      <div className="rp-rpe-bar">
                        {Array.from({ length: 10 }, (_, bi) => {
                          const t = bi / 9,
                            r = Math.round(t < 0.5 ? 2 * t * (224 - 96) + 96 : 224),
                            g = Math.round(t < 0.5 ? 168 : 168 - 2 * (t - 0.5) * 88)
                          return (
                            <div
                              key={bi}
                              className="rp-rpe-seg"
          ...
          {presence === 'Presente' && wodBlocks.length === 0 && (
            <div style={{ fontSize: 12, color: '#806850', marginBottom: 10, padding: '10px 0' }}>
              Nenhum bloco WOD nesta sessão.
            </div>
          )}
          {/* Coach note */}
          <div style={{ marginBottom: 10 }}>
            <button
              ...
              style={{
                ...
                color: showNote ? 'var(--theme-accent)' : '#806850',
                borderColor: showNote ? 'var(--theme-accent)' : '#2a231c',
              }}
          ...
          <div style={{ marginBottom: 12 }}>
            <button
              ...
              className="b bsm"
              style={{
                background: flag ? '#3a1010' : 'transparent',
                color: flag ? '#e05050' : '#806850',
                borderColor: flag ? '#601818' : '#2a231c',
              }}
              onClick={() => setFlag(f => !f)}
            >
            ...
            {hasResult && delConfirm && (
              <>
                ...
                <button
                  type="button"
                  className="b bd bsm"
                  style={{ minWidth: 40, padding: '6px 8px', fontSize: 11 }}
                  onClick={() => {
                    deleteResult(selAthlete.id)
                    setDelConfirm(false)
                  }}
                >
                  Sim
