...
export function LeaderboardView({ athletes, sessions, results }) {
  ...
  const wodResults = useMemo(() => {
    ...
    return results
      .filter(
        r =>
          r.date === selObj.dateKey && r.sessionId === selObj.sessId && r.presence === 'Presente',
      )
      .map(r => {
  ...
  const filtered =
    scaleFilter === 'Todos' ? wodResults : wodResults.filter(r => r.scale === scaleFilter)
  ...
  const doExport = async () => {
    ...
    const lbl = selObj
      ? `${selObj.dt}-${selObj.blLabel}-${scaleFilter}`.replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase()
      : 'leaderboard'
    a.download = `eagles-leaderboard-${lbl}.png`
    ...
    inp.onchange = e => {
      ...
      reader.onload = ev => {
        try {
          ...
          if (src.mobileEaglesBg || src.eaglesBg)
            merged.eaglesBg = src.mobileEaglesBg || src.eaglesBg
          if (src.mobileMegaManBg || src.megaManBg)
            merged.megaManBg = src.mobileMegaManBg || src.megaManBg
          saveSettings(merged)
          ...
          alert('Config carregada! A página irá recarregar.')
      ...
      <div className="sc-card" style={{ padding: 12, marginBottom: 10 }}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button type="button" className="b bsm" onClick={handleLoadConfig}>
              <i className="ti ti-upload" /> Carregar config
            </button>
            <button type="button" className="b bsm" onClick={handleSaveConfig}>
        ...
        <div className="g2">
            ...
            <select value={selWod} onChange={e => setSelWod(e.target.value)}>
              ...
              {wodList.map(w => (
                <option key={w.key} value={w.key}>
                  {w.dt}
                  {w.sessName ? ' (' + w.sessName + ')' : ''} — {w.blLabel}
                  {w.meta ? ' · ' + w.meta : ''}
                </option>
              ))}
            </select>
          ...
          <div className="fg">
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {scales.map(s => {
                const on = scaleFilter === s,
                  accent = APP_CONFIG.themeAccent || '#4ac8c0'
                return (
                  <button
                    key={s}
                    type="button"
                    ...
                    style={{
                      ...
                      color: on ? APP_CONFIG.themeAccentText || '#000' : '#888',
      ...
      {selObj ? (
        <div>
          <div
            ref={imgRef}
            style={{
              background: LB_IMG.bg,
              width: 1080,
              transform: `scale(${Math.min(1, (window.innerWidth - 28) / 1080)})`,
              transformOrigin: 'top left',
              marginBottom: `${-1080 * (1 - Math.min(1, (window.innerWidth - 28) / 1080))}px`,
            }}
          >
            <div
              style={{
                background: LB_IMG.hdrBg,
                padding: '20px 28px 16px',
                borderBottom: `3px solid ${LB_IMG.hdrBorder}`,
              }}
            >
              <div
                style={{
              ...
              >
                {selObj.dt} · {selObj.blLabel}
                {selObj.meta ? ' · ' + selObj.meta : ''}
                {scaleFilter !== 'Todos' ? ' · ' + scaleFilter : ''}
              </div>
            </div>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        gap: 16,
                        padding: '12px 28px',
                        borderBottom: `1px solid ${LB_IMG.divider}`,
                        background: isPodium
                          ? LB_IMG.podiumBg[ri]
                          : ri % 2 === 0
                            ? LB_IMG.rowAlt
                    ...
                    >
                      <div
                        style={{
                          ...
                          fontSize: 18,
                          fontWeight: 900,
                          color: pColor || LB_IMG.rank,
                          width: 32,
                          flexShrink: 0,
                          textAlign: 'center',
                        }}
                      >
                        {isPodium ? podLabels[ri] : `${ri + 1}º`}
                      </div>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <div
                          style={{
                            ...
                            height: 10,
                            borderRadius: '50%',
                            background: ath?.color || LB_IMG.rank,
                            flexShrink: 0,
                          }}
                        />
                        <span
                      ...
                      {r.scale && r.scale !== '-' && (
                        <span
                          style={{
                            ...
                            border: `1px solid ${scaleColor(r.scale)}`,
                            padding: '2px 8px',
                            flexShrink: 0,
                          }}
                        >
