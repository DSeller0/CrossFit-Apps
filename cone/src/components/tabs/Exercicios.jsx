...
function diffExercise(orig, origBlocks, next, nextBlocks) {
  ...
  const cargasStr = ex => {
    const d = ex?.defaults || {}; const p = [];
    if (d.sets) p.push(d.sets + ' séries');
    if (d.dist) p.push(d.dist + (d.distUnit || 'm')); else if (d.reps) p.push(d.reps + ' reps');
  ...
  if (!!orig?.videoPublished !== !!next.videoPublished && (next.videoUrl || orig?.videoUrl))
    ch.push({ label: 'Vídeo publicado', kind: 'changed', before: orig?.videoPublished ? 'ON' : 'OFF', after: next.videoPublished ? 'ON' : 'OFF' });
...
export default function ExerciciosTab() {
  ...
  const allEx = useMemo(() => {
    ...
    BLOCK_ORDER.forEach(b => {
      (registry[b] || []).forEach(ex => { const key = normExName(getExName(ex)); if (!map[key]) map[key] = ex; });
    });
    return Object.values(map).sort((a, b) => getExName(a).localeCompare(getExName(b), 'pt'));
  }, [registry]);

  const matchesSearch = ex => { const q = normExName(search); return !q || normExName(getExName(ex)).includes(q); };
  ...
  const todosSections = useMemo(() => {
    ...
    return FAMILY_GROUPS.flatMap(f => f.cats).map(cat => ({
      cat, color: famColorOf(cat), exs: (registry[cat] || []).filter(matchesSearch),
  ...
  const toggleFamily = key => setOpenFamilies(prev => {
  ...
  const goToEx = exObj => {
    ...
    setDetail({
      orig: o, origName: name, origBlocks: blocks, isNew: false,
      name, selectedBlocks: [...blocks],
      videoUrl:       o.videoUrl       || '',
  ...
  const startAdd = () => {
    setDetail({
      orig: null, origName: '', origBlocks: [], isNew: true,
      name: '', selectedBlocks: selBlock ? [selBlock] : [],
      videoUrl: '', videoPublished: false, description: '', muscles: '', notes: '',
  ...
  const saveDetail = () => {
    if (!detail) return;
    const { orig, origName, origBlocks, videoUrl, videoPublished, description, muscles, notes, selectedBlocks, defaults } = detail;
    const name = detail.name.trim();
    if (!name) { setDetail(p => ({ ...p, error: 'Informe um nome' })); return; }
    if (selectedBlocks.length === 0) { setDetail(p => ({ ...p, error: 'Selecione pelo menos um tipo' })); return; }

    const nameKey = normExName(name), origKey = normExName(origName);
    ...
    if (nameKey !== origKey && selectedBlocks.some(b => (registry[b] || []).some(e => normExName(getExName(e)) === nameKey))) {
      setDetail(p => ({ ...p, error: `"${name}" já existe` })); return;
    }
    ...
    const reg = { ...registry };
    origBlocks.forEach(b => { reg[b] = (reg[b] || []).filter(e => normExName(getExName(e)) !== origKey); });
    ...
    setDetail(p => ({ ...p, orig: newEx, origName: name, origBlocks: [...selectedBlocks], isNew: false, error: undefined }));
  ...
  const renderPane1 = () => (
    <div className={s.p1scroll}>
      <button type="button" className={`${s.todosRow}${selBlock === null ? ' ' + s.todosSel : ''}`} onClick={() => goToType(null)}>
        ...
        return (
          <div key={fam.key} className={s.fam} style={{ '--fc': fam.color }}>
            {isMobile
              ...
              : (
                <button type="button" className={s.famHead} onClick={() => toggleFamily(fam.key)} aria-expanded={open}>
                  <span className={s.famChev}>{open ? '▾' : '▸'}</span>
                  <span className={s.famName}>{fam.label}</span>
                  <span className={s.famCount}>{famCount}</span>
                </button>
              )}
            {open && fam.cats.map(cat => {
              const list = registry[cat] || [];
              const sel = selBlock === cat;
              return (
                <button key={cat} type="button" className={`${s.catRow}${sel ? ' ' + s.catSel : ''}`} onClick={() => goToType(cat)}>
                  <span className={s.catDot} />
                  <span className={s.catBody}>
                    <span className={s.catTop}><span className={s.catName}>{cat}</span><span className={s.catCount}>{list.length}</span></span>
  ...
  const ExCard = ({ ex, catContext }) => {
    ...
    return (
      <button type="button" className={`${s.exCard}${active ? ' ' + s.exCardActive : ''}`}
        style={{ '--fc': famColorOf(catContext || blocksOf(name)[0]) }}
        onClick={() => goToEx(ex)} title={`${filled} de 5 campos preenchidos`}>
        <span className={s.exCardName}>{name}</span>
        <CmplIcons c={c} />
        {tags.length > 0 && (
          <div className={s.chips}>
            {tags.slice(0, 3).map(t => (
              <span key={t} className={s.chip} style={{ '--cc': famColorOf(t) }}>{t}</span>
            ))}
      ...
      <div className={s.pane2} style={{ '--fc': famColor }}>
        <div className={s.stickyHead}>
          <div className={s.p2title}>
            ...
            <span className={s.p2name} style={selBlock ? { color: famColor } : undefined}>{selBlock || 'Todos'}</span>
          ...
          <div className={s.searchBox}>
            ...
            <input className={s.searchInput} type="text" value={search} placeholder="Buscar exercício..."
              aria-label="Buscar exercício" onChange={e => setSearch(e.target.value)} />
            {search && (
        ...
        <div className={s.p2scroll}>
          {selBlock === null ? (
            todosSections.length === 0
              ? <div className={s.empty}>{search ? 'Nenhum exercício encontrado.' : 'Nenhum exercício cadastrado.'}</div>
              : todosSections.map(sec => (
                <div key={sec.cat} className={s.section}>
                  <div className={s.vgLbl} style={{ color: sec.color }}>{sec.cat} <span className={s.vgCount}>({sec.exs.length})</span></div>
                  <div className={s.grid}>
                    {sec.exs.map(ex => <ExCard key={sec.cat + ':' + getExName(ex)} ex={ex} catContext={sec.cat} />)}
                  </div>
                </div>
              ))
          ) : renderCategoryGrid()}
  ...
  const renderCategoryGrid = () => {
    if (visibleExs.length === 0)
      return <div className={s.empty}>{search ? 'Nenhum exercício encontrado.' : 'Nenhum exercício. Use "+ Adicionar".'}</div>;
    const { groups, singles } = groupByRoot(visibleExs, getExName);
    return (
      <>
        {groups.map(g => (
          <div key={g.root} className={s.section}>
            <div className={s.vgLbl}>▾ {g.root} <span className={s.vgCount}>({g.items.length})</span></div>
            <div className={s.grid}>{g.items.map(ex => <ExCard key={getExName(ex)} ex={ex} catContext={selBlock} />)}</div>
          </div>
        ))}
        {singles.length > 0 && (
          <div className={s.section}>
            {groups.length > 0 && <div className={s.vgLbl}>· Avulsos <span className={s.vgCount}>({singles.length})</span></div>}
            <div className={s.grid}>{singles.map(ex => <ExCard key={getExName(ex)} ex={ex} catContext={selBlock} />)}</div>
          </div>
        )}
      </>
    );
  ...
  const renderPane3 = () => {
    if (!detail) return <div className={s.detailEmpty}>Selecione um exercício ou use “+ Adicionar”.</div>;

    const cDetail = completeness(detail);
    const filled = [cDetail.cargas, cDetail.video !== 'none', cDetail.desc, cDetail.musc, cDetail.det].filter(Boolean).length;
    const cls = { on: s.cmplOn, off: s.cmplOff, none: s.cmplNone, unpub: s.cmplUnpub, pub: s.cmplPub };
    const fIco = (mkey) => {
    ...
    const toggleTag = block => setDetail(p => {
      ...
      <div className={s.pane3}>
        ...
        <div className={s.detailBody}>
          <div className={s.fSpan}>
            <Input label="Nome" value={detail.name} autoFocus={detail.isNew}
          ...
          <div className={s.field}>
            ...
            <div className={s.defRow}>
              <input className={s.defInput} style={{ width: 58 }} placeholder="Séries" value={detail.defaults.sets} aria-label="Séries padrão"
                onChange={e => setDetail(p => ({ ...p, defaults: { ...p.defaults, sets: e.target.value } }))} />
              <span className={s.defTimes}>×</span>
              {detail.defaultsDistMode ? (
                <>
                  <input className={s.defInput} style={{ width: 76 }} placeholder="Dist." value={detail.defaults.dist} aria-label="Distância padrão"
                    onChange={e => setDetail(p => ({ ...p, defaults: { ...p.defaults, dist: e.target.value } }))} />
                  <select className={s.defInput} style={{ width: 62 }} value={detail.defaults.distUnit} aria-label="Unidade de distância"
                    onChange={e => setDetail(p => ({ ...p, defaults: { ...p.defaults, distUnit: e.target.value } }))}>
                    <option value="m">m</option><option value="cal">cal</option>
                  </select>
                </>
              ) : (
                <input className={s.defInput} style={{ width: 76 }} placeholder="Reps" value={detail.defaults.reps} aria-label="Reps padrão"
                  onChange={e => setDetail(p => ({ ...p, defaults: { ...p.defaults, reps: e.target.value } }))} />
              )}
              <button type="button" className={`${s.distToggle}${detail.defaultsDistMode ? ' ' + s.on : ''}`}
                onClick={() => setDetail(p => ({ ...p, defaultsDistMode: !p.defaultsDistMode, defaults: { ...p.defaults, ...(p.defaultsDistMode ? { dist: '' } : {}) } }))}
            ...
            <IntensityInput
              ...
              onChange={ins => setDetail(p => ({ ...p, defaults: { ...p.defaults, intensity: ins } }))}
          ...
          <div className={s.field}>
            ...
            <textarea className={s.taInput} rows={2} placeholder="Descrição breve do movimento..." value={detail.description}
              onChange={e => setDetail(p => ({ ...p, description: e.target.value }))} />
          </div>

          <div className={s.field}>
            <div className={s.sLabel}>{fIco('musc')} Músculo(s) alvo</div>
            <input className={s.videoInput} placeholder="Ex: Quadríceps, glúteos." value={detail.muscles}
              onChange={e => setDetail(p => ({ ...p, muscles: e.target.value }))} />
          </div>

          <div className={s.fSpan}>
            <div className={s.sLabel}>{fIco('det')} Detalhe</div>
            <textarea className={s.taInput} rows={2} placeholder="Observações, cuidados ou pontos de atenção..." value={detail.notes}
        ...
        <div className={s.detailFoot}>
          <Button variant="primary" full onClick={saveDetail}>{detail.isNew ? 'Criar exercício' : 'Salvar'}</Button>
          {!detail.isNew && (
    ...
    <div className={s.overlay} onClick={() => setShowVideoModal(false)}>
      <div className={s.modal} role="dialog" aria-modal="true" aria-label={detail.name} onClick={e => e.stopPropagation()}>
        <div className={s.modalHead}>
          ...
          <Button variant="ghost" size="sm" iconOnly onClick={() => setShowVideoModal(false)} aria-label="Fechar"><i className="ti ti-x" /></Button>
        </div>
        <div className={s.videoBox}>
          <iframe className={s.videoFrame} src={`https://www.youtube-nocookie.com/embed/${videoId}?rel=0&modestbranding=1&autoplay=1`} allowFullScreen allow="autoplay" />
        </div>
      </div>
    </div>
  ) : null;
  ...
  const KIND = { created: { lbl: 'Criado', cls: s.diffCreated }, changed: { lbl: 'Alterado', cls: s.diffChanged }, removed: { lbl: 'Removido', cls: s.diffRemoved } };
  const SaveModal = saveSummary ? (
    <div className={s.overlay} onClick={() => setSaveSummary(null)}>
      <div className={s.modal} role="dialog" aria-modal="true" aria-label="Resumo do salvamento" onClick={e => e.stopPropagation()}>
        <div className={s.modalHead}>
          <span className={s.modalTitle}>{saveSummary.isNew ? 'Exercício criado' : 'Alterações salvas'}: {saveSummary.name}</span>
        </div>
        <div className={s.diffBody}>
          {saveSummary.changes.length === 0
            ? <div className={s.diffEmpty}>Nenhuma alteração — nada mudou desde o último salvamento.</div>
            : saveSummary.changes.map((c, i) => (
              <div key={i} className={s.diffRow}>
                ...
                <span className={s.diffVal}>
                  {c.kind === 'changed' ? <><span className={s.diffBefore}>{c.before}</span> → <span className={s.diffAfter}>{c.after}</span></>
                    : c.kind === 'removed' ? <span className={s.diffBefore}>{c.before}</span>
        ...
        <div className={s.diffFoot}>
          <Button variant="primary" full onClick={() => setSaveSummary(null)}>OK</Button>
        </div>
      </div>
    </div>
  ) : null;
  ...
  const DeleteConfirm = (
    <ConfirmReview open={!!pendingDelete} title="Remover exercício" editLabel="Cancelar" confirmLabel="Remover"
      onEdit={() => setPendingDelete(null)} onClose={() => setPendingDelete(null)} onConfirm={confirmDelete}>
      <div style={{ fontSize: 13, color: 'var(--sub)', lineHeight: 1.5 }}>
        Remover <strong style={{ color: 'var(--cream)' }}>{pendingDelete}</strong> de {deleteBlocks.length} tipo{deleteBlocks.length !== 1 ? 's' : ''}?
        Vídeo, cargas padrão e descrição serão perdidos.
      </div>
    </ConfirmReview>
  );
  ...
  if (isMobile) {
    const BackBtn = ({ label }) => (
      <button type="button" className={s.backBtn} onClick={goBack}><i className="ti ti-chevron-left" /> {label}</button>
    );
    return (
      <div className={s.mobileWrap}>
        {pane === 0 && renderPane1()}
        {pane === 1 && <div className={s.mobilePane}><BackBtn label="Categorias" />{renderPane2()}</div>}
        {pane === 2 && <div className={s.mobilePane}><BackBtn label={selBlock || 'Todos'} />{renderPane3()}</div>}
        <Footer />
        {VideoModal}{SaveModal}{DeleteConfirm}
      </div>
    );
  }
