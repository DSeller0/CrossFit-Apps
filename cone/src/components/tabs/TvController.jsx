...
export default function TvController({ sessions: propSessions }) {
  ...
  const loadResults = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq('date', selDate)
    if (data) setResults(data.map(rowToResult))
  }, [selSessId, selDate])
  useEffect(() => {
  ...
  const liveReg = useLiveRegistration({
    ...
    selDate,
    selSessObj,
    activeClassId: classes.activeClass?.id,
    elapsedSecs: timer.elapsedSecs,
    currentTimerBlock: timer.currentTimerBlock,
    loadResults,
  })
  ...
  const rotation = useGroupRotation({
    ...
    selSessObjRef,
    activeClassRef,
    setTimerBlkId: timer.setTimerBlkId,
    setTimerType: timer.setTimerType,
    setTimerCap: timer.setTimerCap,
    timerStartedAt: tv?.timer_started_at,
    timerBlockId: tv?.timer_block_id,
    rotationRestUntil: tv?.rotation_rest_until,
  ...
  const rotationBlocks =
    rotationBlockIds.length > 0 ? wodBlocks.filter(b => rotationBlockIds.includes(b.id)) : wodBlocks
  ...
  const previewTv = useMemo(
    () => ({
      ...
      timer_cap_secs: timer.timerCap * 60,
      timer_block_id: timer.timerBlkId,
    }),
  ...
  return (
    <div className={st.page}>
      ...
      <div className={st.hdr}>
        <div>
          ...
          <div className={st.hdrSub}>
            {tv?.slide && tv.slide !== 'blank' ? (
              <span className={st.hdrLive}>
                ● TV ativa · {SLIDES.find(s => s.id === tv.slide)?.lbl}
              </span>
            ) : (
        ...
        <button
          className={`${st.btn} ${st.openBtn}`}
          onClick={() => window.open('/CrossFit-Apps/tv.html', '_blank')}
        >
        ...
        {dayS.length === 0 ? (
          ...
          <div className={st.sessChips}>
            {dayS.map(sess => {
              ...
              const blocks = (sess.blocks || []).filter(
      ...
      <div className={st.layout2}>
        ...
        <div className={st.leftPane}>
          <ClassPanel
            ...
            setClassLabel={classes.setClassLabel}
            startClass={classes.startClass}
            endClass={classes.endClass}
            liveReg={liveReg}
          />
        </div>
        ...
        <div className={st.rightGrid}>
          <div className={st.rightStack}>
            ...
            <div className={st.card}>
              ...
              <div className={st.slideRow}>
                {SLIDES.map(({ id, icon, lbl }) => (
                  <button
                    key={id}
                    className={`${st.btn} ${st.slideBtn} ${slide === id ? st.sel : ''}`}
              ...
              <div className={st.qrRow}>
                ...
                {(() => {
                  ...
                  return (
                    <button
                      className={`${st.qrToggle} ${qrOn ? st.on : st.off}`}
                      onClick={() => push({ show_qr: !qrOn })}
                    >
                      {qrOn ? 'Ativo' : 'Oculto'}
                    </button>
                  )
            ...
            <div className={st.card}>
              ...
              {groups.length > 0 ? (
                  ...
                  return (
                    <div style={{ marginBottom: 12 }}>
                      <div className={st.timerHint}>
                        ...
                        <strong className={st.timerHintStrong}>
                          {Math.ceil(totalSecs / 60)} min total
                        </strong>
                        {finishers.length > 0 && (
                          <span className={st.timerHintDim}>
                            {' '}
                            → {finishers.map(b => b.label || b.type).join(' + ')} (finisher)
                          </span>
                        )}
                      </div>
                      <div className={st.timerGrid} style={{ gridTemplateColumns: '1fr 1fr' }}>
                        <div>
                          <label className={st.lbl}>
                            Tipo <span className={st.timerGridBadge}>(do bloco)</span>
                          </label>
                          <div className={st.roInput}>{rotType}</div>
                        </div>
                        <div>
                          <label className={st.lbl}>Cap (min)</label>
                          <div className={st.roInput}>{rotCap}</div>
                        </div>
                      </div>
                    </div>
                  )
                ...
                <>
                  ...
                  <div
                    className={st.timerGrid}
                    style={{
                      gridTemplateColumns:
                        timer.timerBlkId && wodBlocks.length > 1 ? '1fr 1fr 1fr' : '1fr 1fr',
                    }}
                  >
                    <div>
                      <label className={st.lbl}>
                        Tipo{' '}
                        {timer.timerBlkId && <span className={st.timerGridBadge}>(do bloco)</span>}
                      </label>
                      {timer.timerBlkId ? (
                        ...
                        <select
                          ...
                          onChange={e => timer.setTimerType(e.target.value)}
                          className={st.input}
                        >
                          {TIMER_TYPES.map(t => (
                    ...
                    <div>
                      ...
                      <input
                        ...
                        min={1}
                        max={120}
                        value={timer.timerCap}
                        onChange={e => timer.setTimerCap(Number(e.target.value))}
                        className={st.input}
                      />
                    </div>
                    {timer.timerBlkId && wodBlocks.length > 1 && (
                      <div>
                        ...
                        <input
                          ...
                          value={restSecs}
                          onChange={e =>
                            push({ rotation_rest_secs: Math.max(0, parseInt(e.target.value) || 0) })
                          }
                          className={st.input}
                        />
                      </div>
              ...
              <div className={st.timerBtns}>
                {!timerRun ? (
                  <button className={`${st.btn} ${st.timerBtnPlay}`} onClick={handleStartTimer}>
                    <i className="ti ti-player-play" /> Iniciar
                  </button>
                ) : (
                  <button className={`${st.btn} ${st.timerBtnPause}`} onClick={timer.pauseTimer}>
            ...
            <GroupsPanel
              ...
              restSecs={restSecs}
              autoAdvance={rotation.autoAdvance}
              setAutoAdvance={rotation.setAutoAdvance}
              athletes={athletes}
              createGroups={rotation.createGroups}
              dissolveGroups={rotation.dissolveGroups}
              setGroupBlock={rotation.setGroupBlock}
              reassignMember={rotation.reassignMember}
              advanceAll={rotation.advanceAll}
              toggleRotationBlock={rotation.toggleRotationBlock}
              push={push}
            />
          </div>
          ...
          <div className={st.previewCard}>
              ...
              <div className={st.previewInner} style={{ transform: `scale(${prevScale})` }}>
                ...
                {slide === 'wod' && (
                  <WodSlide
                    ...
                    tv={previewTv}
                    gymName={gymName}
                    classExecs={classes.todayClasses}
                    athletes={athletes}
                  />
                )}
                {slide === 'timer' && (
            ...
            <div className={st.previewFoot}>
              Slide atual na TV:{' '}
              <strong className={st.previewFootStrong}>
                {SLIDES.find(s => s.id === slide)?.lbl || '—'}
              </strong>
            </div>
          </div>
        </div>
