      ...
      <input
        ...
        onKeyDown={e => {
          ...
          if (e.key === 'ArrowDown' && open && suggestions.length)
            ref.current?.querySelector('.ex-suggestion')?.focus()
        }}
      />
      {open &&
              ...
              <div
                ...
                onKeyDown={e => {
                  ...
                  if (e.key === 'ArrowUp') {
                    ...
                    p ? p.focus() : ref.current?.querySelector('input')?.focus()
                  }
                  if (e.key === 'Escape') {
                    setOpen(false)
              ...
              >
                <span
                  style={{
                    ...
                    height: 7,
                    borderRadius: '50%',
                    background: ECOL[s.blockType]?.text || MUTED,
                    flexShrink: 0,
...
function PrRow({ pr, onAddResult, onEdit, onDelete, showActions }) {
  ...
  const bestLabel = best
    ? pr.type === 'load'
      ? `${best.value} ${pr.unit || 'kg'}`
      : pr.type === 'reps'
        ? `${best.value} reps`
        : best.value
    : '—'
  const targetLabel = pr.target
    ? pr.type === 'load'
      ? `${pr.target} ${pr.unit || 'kg'}`
      : pr.type === 'reps'
        ? `${pr.target} reps`
        : pr.target
    : null
  const bestDate = best
    ? new Date(best.date + 'T12:00:00').toLocaleDateString('pt-BR', {
        day: '2-digit',
        month: '2-digit',
      })
    : null
  ...
  const bar =
    pct !== null ? (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {fill > 0 && (
                  <div
                    style={{
                      ...
                      bottom: 0,
                      width: `${fill * 100}%`,
                      background: fill === 1 ? 'var(--theme-accent)' : 'var(--theme-accent)88',
                    }}
                  />
                )}
              </div>
        ...
        {targetLabel && (
          <div style={{ fontSize: 9, color: DIM, textAlign: 'right' }}>Meta: {targetLabel}</div>
        )}
      </div>
    ) : null
        ...
        {delta && (
          <div
            style={{
              ...
              minWidth: 46,
              textAlign: 'right',
              color: delta.good === true ? '#68d8a0' : delta.good === false ? '#e05848' : MUTED,
            }}
          >
        ...
        {showActions && (
          <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
            <button
              type="button"
              className="b bsm"
              style={{ padding: '3px 6px', minHeight: 22, fontSize: 11 }}
              onClick={onAddResult}
            >
            ...
            <button
              type="button"
              className="b bd bsm"
              style={{ padding: '3px 6px', minHeight: 22, fontSize: 11, opacity: 0.6 }}
              onClick={onEdit}
            >
            ...
            <button
              type="button"
              className="b bd bsm"
              style={{ padding: '3px 6px', minHeight: 22, fontSize: 11, opacity: 0.5 }}
              onClick={onDelete}
            >
...
function AddResultModal({ pr, onSave, onClose }) {
  ...
  const bestLabel =
    pr.type === 'time'
      ? best?.value
      : best?.value
        ? best.value + (pr.unit ? ' ' + pr.unit : '')
        : '—'
  const isPR =
    value &&
    (pr.type === 'time' ? toSecs(value) < toSecs(best?.value) : Number(value) > Number(best?.value))
      ...
      <div className="settings-modal" style={{ maxWidth: 340 }} onClick={e => e.stopPropagation()}>
        <div className="settings-drag-hdr">
          ...
          <button
            type="button"
            className="b bd bsm"
            style={{ marginLeft: 'auto', padding: '3px 8px', minHeight: 24 }}
            onClick={onClose}
          >
            <i className="ti ti-x" />
          </button>
        ...
        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {best && (
            ...
            >
              Melhor atual: {bestLabel}
            </div>
          )}
          <div className="g2">
            <div className="fg">
              <span className="lbl">
                {pr.type === 'time' ? 'Tempo (mm:ss)' : pr.type === 'reps' ? 'Reps' : 'Carga'}
              </span>
              <input
                className="ex-input"
                placeholder={pr.type === 'time' ? '03:45' : pr.type === 'reps' ? '25' : '120'}
                value={value}
          ...
          {isPR && (
            <div style={{ fontSize: 12, color: '#d8a840', fontWeight: 700, textAlign: 'center' }}>
              🏆 Novo PR!
            </div>
          )}
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button type="button" className="b bd bsm" onClick={onClose}>
              Cancelar
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div className="fg">
            ...
            <ExerciseCombobox
              ...
              excludeNames={
                isEdit
                  ? (existingNames || []).filter(n => n.toLowerCase() !== editPr.name.toLowerCase())
                  : existingNames
              }
            />
          </div>
              ...
              {exBlocks.map(bt => (
                <span
                  ...
                  style={{
                    ...
                    fontWeight: 700,
                    padding: '2px 7px',
                    background: ECOL[bt]?.bg || STONE,
                    color: ECOL[bt]?.text || MUTED,
                    border: `1px solid ${ECOL[bt]?.text || DIV}44`,
                  }}
                >
                  {bt}
                </span>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    background: type === t ? 'var(--theme-accent)' : 'transparent',
                    color: type === t ? 'var(--theme-accent-text)' : MUTED,
                    borderColor: type === t ? 'var(--theme-accent)' : DIV,
                  }}
                  onClick={() => setType(t)}
                >
                  {lbl}
          ...
          {!isEdit && (
            <div className="g2">
              <div className="fg">
                <span className="lbl">
                  {type === 'time' ? 'Tempo (mm:ss)' : type === 'reps' ? 'Reps' : 'Carga'}
                </span>
                <input
                  className="ex-input"
                  placeholder={type === 'time' ? '03:45' : type === 'reps' ? '25' : '120'}
                  value={value}
                  onChange={e => setValue(e.target.value)}
                />
              </div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {['kg', 'lb'].map(u => (
                      <button
                        ...
                        style={{
                          flex: 1,
                          background: unit === u ? 'var(--theme-accent)' : 'transparent',
                          color: unit === u ? 'var(--theme-accent-text)' : MUTED,
                          borderColor: unit === u ? 'var(--theme-accent)' : DIV,
                        }}
                        onClick={() => setUnit(u)}
                      >
                        {u}
          ...
          <div className="fg">
            ...
            <input
              className="ex-input"
              placeholder={type === 'time' ? '03:00' : type === 'reps' ? '30' : '140'}
              value={target}
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button
              ...
              className="b bsec"
              style={{ flex: 1 }}
              disabled={!name.trim() || (!isEdit && !value)}
              onClick={save}
            >
    ...
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <button
          type="button"
          className="b bsm"
          style={{ padding: '3px 7px', minHeight: 22, fontSize: 11 }}
          onClick={() => {
            if (window.confirm(`Confirmar sessão para "${goal.name}"?`)) onAddSession()
          }}
        >
          +1
        </button>
        ...
        <button
          ...
          style={{ padding: '3px 6px', minHeight: 22, fontSize: 11, opacity: 0.6 }}
          onClick={() => {
            if (window.confirm(`Remover "${goal.name}"?`)) onDelete()
          }}
        >
          <i className="ti ti-trash" />
        </button>
              ...
              {fill > 0 && (
                <div
                  style={{
                    ...
                    bottom: 0,
                    width: `${fill * 100}%`,
                    background: fill === 1 ? color : color + '99',
                    transition: 'width .3s',
                  }}
                />
              )}
            ...
            (goal.milestones || []).map((m, mi) => (
              <div
                ...
                style={{
                  ...
                  gap: 8,
                  padding: '4px 0',
                  borderBottom: mi < goal.milestones.length - 1 ? `1px solid ${DIV}` : 'none',
                }}
              >
                <input
                  type="checkbox"
                  checked={!!m.hit}
                  onChange={() => onMilestoneHit(mi, !m.hit)}
                ...
                <span
                  style={{
                    fontSize: 12,
                    color: m.hit ? DIM : SUB,
                    textDecoration: m.hit ? 'line-through' : 'none',
                    flex: 1,
                  }}
                >
                  {m.label}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div>
        ...
        >
          <span
            style={{ fontSize: 9, color: DIM, textTransform: 'uppercase', letterSpacing: '.06em' }}
          >
            Milestones (máx. 5)
          </span>
          {milestones.length < 5 && (
        ...
        {milestones.map((m, i) => (
          <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 5 }}>
            ...
            <button
              type="button"
              className="b bd bsm"
              style={{ padding: '2px 6px', minHeight: 22, fontSize: 11, opacity: 0.6 }}
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
        ...
        <button type="button" className="b bd bsm" onClick={onCancel}>
          Cancelar
        </button>
      </div>
    </div>
...
export default function AtletasTab({ sessions, results }) {
  ...
  const goToAthlete = athId => {
    ...
    setProfileForm({
      ...
      notes: a.notes || '',
      color: a.color || '#e87820',
      since: a.since || todayISO(),
    })
  ...
  const deleteGoal = goalId => {
    persist({
      ...
      athleteGoals: {
        ...(goalsData.athleteGoals || {}),
        [selAthlete]: athGoals.filter(g => g.id !== goalId),
      },
    })
  }
  ...
  const hitMilestone = (goalId, mi, hit) => {
    ...
    updateGoal(goalId, {
      milestones: g.milestones.map((m, i) =>
        i === mi ? { ...m, hit, hitDate: hit ? m.hitDate || todayISO() : undefined } : m,
      ),
    })
  }
  ...
  const addResult = (prId, result) => {
    const updated = athPrs.map(p =>
      p.id !== prId ? p : { ...p, results: [...p.results, result].slice(-5) },
    )
    persist({ ...goalsData, prs: { ...(goalsData.prs || {}), [selAthlete]: updated } })
  ...
  const deletePr = prId => {
    ...
    persist({
      ...goalsData,
      prs: { ...(goalsData.prs || {}), [selAthlete]: athPrs.filter(p => p.id !== prId) },
    })
  }
    ...
    Object.keys(sessions || {})
      ...
      .forEach(date => {
        ;(sessions[date] || []).forEach(s => {
          if (matchesAthlete(s, ath.name)) all.push({ date, session: s })
        })
      })
    return [
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {athletes.map(a => {
          ...
          return (
            <div
              key={a.id}
              onClick={() => goToAthlete(a.id)}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 9,
                padding: '11px 14px',
                background: isSel ? STONE : 'transparent',
            ...
            >
              <span
                style={{ width: 9, height: 9, borderRadius: '50%', background: col, flexShrink: 0 }}
              />
              <span
                style={{
                  flex: 1,
              ...
              {pct !== null && (
                <span style={{ fontSize: 10, color: isSel ? MUTED : DIM }}>{pct}%</span>
              )}
              {isMobile && (
                <i
                  className="ti ti-chevron-right"
                  style={{ color: DIM, fontSize: 12, flexShrink: 0 }}
                />
              )}
            </div>
          )
  ...
  const renderDetail = () => {
    ...
    const sortedGroupEntries = [
      ...
      ...Object.keys(groupedPrs)
        .filter(k => !blockOrder.includes(k))
        .map(k => [k, groupedPrs[k]]),
    ]
        ...
        >
          ...
          >
            <div>
              ...
              >
                ...
                {ath.since && (
                  <span style={{ fontSize: 10, color: DIM }}>
                    desde{' '}
                    {new Date(ath.since + 'T12:00:00').toLocaleDateString('pt-BR', {
                      month: 'short',
                      year: 'numeric',
                    })}
                  </span>
              ...
              {ath.notes && (
                <div style={{ fontSize: 11, color: DIM, marginTop: 6, fontStyle: 'italic' }}>
                  {ath.notes}
                </div>
              )}
            </div>
          ...
          <div>
            ...
            {sessionStrip.length === 0 ? (
              ...
              sessionStrip.map(({ date, session }) => {
                ...
                const wb = myResult
                  ? (myResult.blocks || []).find(b =>
                      isWodBlock({ type: b.blockType, label: b.blockLabel }),
                    )
                  : null
                const perf = wb
                  ? wb.perfTime || (wb.perfRounds ? wb.perfRounds + 'rds' : null)
                  : null
                return (
                  <div
                    key={date + '|' + session.id}
                  ...
                  >
                    <div
                      style={{
                        width: 3,
                        alignSelf: 'stretch',
                        background: isToday ? 'var(--theme-accent)' : isPast ? DIM : DIV + '66',
                        flexShrink: 0,
                      }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
                        style={{
                          fontSize: 12,
                          fontWeight: 700,
                          color: isToday ? CREAM : SUB,
                          overflow: 'hidden',
                          textOverflow: 'ellipsis',
                          whiteSpace: 'nowrap',
                        }}
                      >
                        {session.sessionName || DAY_PT_TITLE[d.getDay()]}
                      </div>
                      <div style={{ fontSize: 10, color: MUTED }}>
                        {DAY_PT_TITLE[d.getDay()]} ·{' '}
                    ...
                    {isPast && !isToday && perf && (
                      <span style={{ fontSize: 11, fontWeight: 700, color: CREAM, flexShrink: 0 }}>
                        {perf}
                      </span>
                    )}
                    {isPast && !isToday && !myResult && (
                      <span style={{ fontSize: 10, color: DIM, flexShrink: 0 }}>—</span>
                    )}
                    {!isPast && (
                      <span style={{ fontSize: 10, color: MUTED, flexShrink: 0 }}>Próxima</span>
                    )}
                  </div>
                )
              })
  ...
  const modals = (
    <>
              ...
              <div className="g2">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div
                      style={{
                        width: 28,
                        height: 28,
                        background: profileForm?.color || '#e87820',
                        border: `2px solid ${DIV}`,
                        cursor: 'pointer',
                        flexShrink: 0,
                      }}
                      onClick={() => document.getElementById('ath-clr')?.click()}
                    />
                    <input
                      type="color"
                      id="ath-clr"
                      value={profileForm?.color || '#e87820'}
                    ...
                    <input
                      ...
                      onChange={e => {
                        if (/^#[0-9a-fA-F]{0,6}$/.test(e.target.value))
                          setProfileForm(f => ({ ...f, color: e.target.value }))
                      }}
                ...
                <div className="fg">
                  ...
                  <input
                    type="date"
                    className="ex-input"
                    value={profileForm?.since || ''}
                    onChange={e => setProfileForm(f => ({ ...f, since: e.target.value }))}
                  />
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          ...
          >
            <div className="settings-drag-hdr">
              ...
              <button
                ...
                className="b bd bsm"
                style={{ marginLeft: 'auto', padding: '3px 8px', minHeight: 24 }}
                onClick={() => setConfiguringGoal(null)}
              >
                <i className="ti ti-x" />
              </button>
            </div>
      ...
      {(showPrModal || editingPr) && (
        <PrModal
          editPr={editingPr || null}
          existingNames={athPrs.map(p => p.name)}
          onSave={savePr}
          onClose={() => {
            setShowPrModal(false)
            setEditingPr(null)
      ...
      {addResultFor && (
        <AddResultModal
          pr={addResultFor}
          onSave={result => addResult(addResultFor.id, result)}
