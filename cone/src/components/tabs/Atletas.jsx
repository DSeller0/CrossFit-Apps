  ...
  return (
    <div ref={ref} style={{ position: 'relative', flex: 1 }}>
      ...
      {open && suggestions.length > 0 && dropRect && (
        <div ref={dropdownRef} style={{ position: 'fixed', top: dropRect.bottom + 2, left: dropRect.left, width: dropRect.width, zIndex: 9999, background: '#1a1a1a', border: '1px solid #333', borderRadius: '5px', maxHeight: '180px', overflowY: 'auto', boxShadow: '0 4px 12px rgba(0,0,0,.5)' }}>
          {suggestions.map((s, i) => (
            <div key={i} className="ex-suggestion" tabIndex={0}
              style={{ padding: '6px 10px', fontSize: '13px', color: '#ddd', cursor: 'pointer', borderBottom: i < suggestions.length - 1 ? '1px solid #222' : 'none', display: 'flex', alignItems: 'center', gap: 8 }}
