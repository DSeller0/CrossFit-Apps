        ...
        <div>
          ...
          <div className={st.groupsCreateBtns}>
            {[2, 3, 4].map(n => (
              <button
                key={n}
                className={`${st.btn} ${st.createBtn}`}
                onClick={() => createGroups(n)}
              >
                {n} grupos
              </button>
            ))}
        ...
        <div>
          <div className={st.groupCards}>
            {groups.map(grp => (
              <div
                key={grp.id}
                className={st.groupCard}
                style={{ borderColor: grp.color + '44', borderLeftColor: grp.color }}
              >
                <div className={st.groupHead}>
                  <span className={st.groupName} style={{ color: grp.color }}>
                    {grp.name}
                  </span>
                  <select
                    className={`${st.input} ${st.groupBlockSelect}`}
                    value={groupPositions[grp.id] || ''}
                ...
                <div className={st.groupMembers}>
                  {[
                    ...(grp.athleteIds || [])
                      ...
                      .filter(Boolean),
                    ...(grp.anonNames || []),
                  ].join(' · ') || <em className={st.groupMembersEmpty}>Sem atletas</em>}
                </div>
              </div>
            ))}
          </div>
            ...
            <div className={st.assignSection}>
              ...
              {[
                ...
                const inGroup = groups.find(g =>
                  m.type === 'real'
                    ? (g.athleteIds || []).includes(m.id)
                    : (g.anonNames || []).includes(m.name),
                )
                return (
                  <div key={m.type === 'real' ? m.id : `anon-${m.name}`} className={st.assignRow}>
                    ...
                    {groups.map(g => (
                      <button
                        key={g.id}
                        className={st.assignBtn}
                        onClick={() => reassignMember(m, g.id)}
                        style={{
            ...
            <div className={st.rotationSection}>
              ...
              <div className={st.rotationChips}>
                {wodBlocks.map(bl => {
                  ...
                  return (
                    <button
                      key={bl.id}
                      className={`${st.btn} ${st.rotationChip} ${inRot ? st.on : ''}`}
              ...
              <div className={st.restRow}>
                ...
                <input
                  ...
                  max="600"
                  value={restSecs}
                  className={`${st.input} ${st.restInput}`}
                  onChange={e =>
          ...
          <div className={st.rotationCtrls}>
            <button className={`${st.btn} ${st.advanceBtn}`} onClick={advanceAll}>
              Avançar todos →
            </button>
            <button
              className={`${st.btn} ${st.autoBtn} ${autoAdvance ? st.on : ''}`}
              onClick={() => setAutoAdvance(a => !a)}
            >
              ⏩ Auto: {autoAdvance ? 'ON' : 'OFF'}
            </button>
            <button className={`${st.btn} ${st.dissolveBtn}`} onClick={dissolveGroups}>
              Dissolve ×
            </button>
          </div>
        </div>
