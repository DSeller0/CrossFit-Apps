...
export function SyncProvider({ children }) {
  ...
  useEffect(() => {
    ...
    syncFromSupabase()
      .then(fresh => {
        if (fresh.sessions) {
          suppressSessionsSave.current = true
          setSessions(fresh.sessions)
        }
        if (fresh.events) {
          suppressEventsSave.current = true
  ...
  useEffect(() => {
    if (!sessionsMounted.current) {
      sessionsMounted.current = true
      return
    }
    if (suppressSessionsSave.current) {
      suppressSessionsSave.current = false
      return
    }
    saveLS(sessions)
  }, [sessions])
  ...
  useEffect(() => {
    ...
    if (suppressEventsSave.current) {
      suppressEventsSave.current = false
      return
    }
    saveEvents(events)
  }, [events])
  ...
  return (
    <SyncContext.Provider
      value={{ sessions, setSessions, events, setEvents, syncState, handleSync }}
    >
      {children}
    </SyncContext.Provider>
  )
