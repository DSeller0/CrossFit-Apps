  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const elapsed = Math.floor(
        (Date.now() - t.timer_started_at) / 1000 + (t.timer_paused_elapsed ?? 0),
      )
        ...
        if (finishers.length > 0) {
          ...
          push({
            ...
            ...(autoAdvanceRef.current
              ? { timer_started_at: Date.now(), timer_paused_elapsed: 0 }
              : {}),
          })
  ...
  async function createGroups(n) {
    ...
    if (wods.length > 0) {
      ...
      newGroups.forEach((g, i) => {
        newPos[g.id] = wods[i % wods.length].id
      })
