...
export async function dbLoadResults() {
  ...
  if (error) {
    console.warn('[supabase] load results_v2', error.message)
    return null
  }
...
export async function dbSaveResults(results) {
  ...
  const { error } = await supabase.from('results_v2').upsert(rows, { onConflict: 'id' })
...
export async function dbGetUpdatedAt(table) {
  const { data } = await supabase.from(table).select('updated_at').eq('id', 1).maybeSingle()
