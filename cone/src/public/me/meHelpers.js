...
export function calcStreak(present) {
  ...
  for (const d of dates) {
    if (d !== cur) break
    s++
    const p = new Date(cur + 'T12:00:00'); p.setDate(p.getDate() - 1); cur = toISO(p)
  }
  return s
}
...
export function calcMaxStreak(present) {
  ...
  for (let i = 1; i < dates.length; i++) {
    ...
    if (gap === 1) { cur++; if (cur > mx) mx = cur } else cur = 1
  }
  return mx
}
...
export function calcBlockStats(sessions, present, name, types, start, end) {
  ...
  present.forEach(r => {
    ...
    ;(r.blocks || []).forEach(b => { if (ts.has(b.blockType)) executed[b.blockType]++ })
  })
