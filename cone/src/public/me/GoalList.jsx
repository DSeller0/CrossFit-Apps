      ...
      <div className={styles.shInner}>
        <h2 className={styles.shTitle}>
          ...
          <span className={styles.shTitleR}>
            {totalMarcosHit} marco{totalMarcosHit !== 1 ? 's' : ''} atingido
            {totalMarcosHit !== 1 ? 's' : ''}
          </span>
        </h2>
          ...
          return (
            <div key={gi} className={styles.goalItem}>
              <div className={styles.goalHdr}>
                ...
                <span
                  className={`${styles.goalPct} ${pct >= 100 ? styles.goalPctDone : styles.goalPctOn}`}
                >
                  {pct}%
                </span>
                {ms.length > 0 && (
                  <span className={styles.goalMarcos}>
                    {hitCount}/{ms.length} marcos
                  </span>
                )}
              </div>
                    ...
                    {ms.map((m, mi) => {
                      ...
                      return (
                        <div key={mi} className={`${styles.msRow} ${cls}`}>
                          <span className={styles.msRowIcon} aria-hidden="true">
                            {m.hit ? '◆' : '◇'}
                          </span>
              ...
              <div className={styles.goalSub}>
                {goal.completedSessions} de {goal.totalSessions} sessões
              </div>
            </div>
          )
        })}
