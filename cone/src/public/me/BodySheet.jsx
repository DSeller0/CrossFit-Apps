...
export default function BodySheet({
  ...
  weight, height, bodyFat, note, warn,
  onWeight, onHeight, onBodyFat, onNote, onSave,
}) {
  ...
  return (
    <Sheet open={open} onClose={onClose} titleId="bodySheetTitle">
      <div className={styles.lsHdr}>
        <h2 className={styles.lsExName} id="bodySheetTitle">Corpo</h2>
        <span className={styles.lsHdrDate}>{fmtDateShort(new Date().toISOString().slice(0, 10))}</span>
      </div>
      ...
      <div className={styles.lsBody}>
        ...
        <div className={styles.lsInpRow}>
          <label className={styles.lsInpWrap}>
            <input type="number" className={styles.lsInp} value={weight} placeholder="—" step="0.1" min="0"
              onChange={e => onWeight(e.target.value)} />
            <span className={styles.lsInpLbl}>Peso (kg)</span>
          </label>
          <label className={styles.lsInpWrap}>
            <input type="number" className={styles.lsInp} value={height} placeholder="—" step="1" min="0" max="300"
              onChange={e => onHeight(e.target.value)} />
            <span className={styles.lsInpLbl}>Altura (cm)</span>
          </label>
          <label className={styles.lsInpWrap}>
            <input type="number" className={styles.lsInp} value={bodyFat} placeholder="—" step="0.1" min="0" max="100"
              onChange={e => onBodyFat(e.target.value)} />
            <span className={styles.lsInpLbl}>Gordura (%)</span>
          </label>
        </div>

        <label className={styles.srOnly} htmlFor="bodyNote">Nota</label>
        <textarea id="bodyNote" className={styles.lsNote} rows="2" placeholder="Nota opcional..."
        ...
        <div className={styles.lsActions}>
          ...
          <button className={styles.lsBtnSave} onClick={onSave}>SALVAR</button>
        </div>
      </div>
    </Sheet>
  )
