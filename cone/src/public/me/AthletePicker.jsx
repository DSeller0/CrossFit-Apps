...
export default function AthletePicker({ variant, athletes, selected, query, onQuery, onSelect, onClear }) {
    ...
    return (
      <div className={styles.selPane}>
        ...
        <div className={styles.selSearch}>
          <input className={styles.selInp} type="search" placeholder="Buscar..." aria-label="Buscar atleta"
            value={query} onChange={e => onQuery(e.target.value)} autoComplete="off" />
        </div>
        <div className={styles.selList}>
          <a className={`${styles.selTodos} ${!selected ? styles.selTodosOn : ''}`} href="me.html"
            aria-current={!selected ? 'true' : undefined}
            onClick={e => { e.preventDefault(); onClear() }}>
            <span className={`${styles.selTodosDi} ${!selected ? styles.selDiOn : ''}`} aria-hidden="true">{!selected ? '◆' : '◇'}</span>
          ...
          {matches.map(a => {
            ...
            return (
              <a key={a.id} className={`${styles.selItem} ${on ? styles.selItemOn : ''}`} href={`me.html?id=${a.id}`}
                ...
                <span className={`${styles.selDi} ${on ? styles.selDiOn : ''}`} aria-hidden="true">{on ? '◆' : '◇'}</span>
  ...
  return (
    <div className={styles.pickerWrap}>
      ...
      <input className={styles.pickerInput} type="search" placeholder="Buscar..." aria-label="Buscar atleta"
        value={query} onChange={e => onQuery(e.target.value)} autoComplete="off" />
        ...
        {!matches.length
          ...
          : matches.map(a => (
            <a key={a.id} className={styles.pickerItem} href={`me.html?id=${a.id}`}
              onClick={e => { e.preventDefault(); onSelect(a) }}>
              <span className={styles.athDot} style={{ background: a.color || 'var(--teal)' }} aria-hidden="true" />
