      ...
      <div className={s.blocks}>
        {blocks.map((b, i) => (
          <SessionBlock
            key={b.id || i}
            block={b}
            expanded={expanded.has(b.id || i)}
            onToggle={() => toggle(b.id || i)}
          />
        ))}
      </div>
      <div className={s.sessFoot}>
        <span className={s.reg}>{countLbl}</span>
        <a className={s.footBtn} href={agendaUrl} title="Ver na agenda" aria-label="Ver na agenda">
          <IconCalendar className={s.footIc} />
          <span className={s.footArr}>→</span>
        </a>
        {!isFuture && (
          <a
            className={`${s.footBtn} ${s.footBtnReg}`}
            href={regUrl}
            title="Registrar resultado"
            aria-label="Registrar resultado"
          >
            <IconChartBar className={s.footIc} />
            <span className={s.footArr}>→</span>
          </a>
        )}
      </div>
...
function SessionBlock({ block, expanded, onToggle }) {
  ...
  const meta = [
    isWodBlock(block)
      ? blkMeta(block)
      : [block.duration && `${block.duration}'`, block.rounds && `${block.rounds} rds`]
          .filter(Boolean)
          .join(' · '),
  ...
  return (
    <div className={`${s.blk}${expanded ? ' ' + s.blkOpen : ''}`}>
      ...
      >
        ...
        <span className={s.blkType} style={{ color }}>
          {block.type}
        </span>
        {label && (
          <>
            <span className={s.blkSep}>·</span>
            <span className={s.blkLabel}>{label}</span>
          </>
        )}
      ...
      {expanded && canExpand && (
        <div className={s.blkBody}>
          <ExerciseList exercises={exes} color={color} size="tiny" />
        </div>
      )}
    </div>
  )
...
export function DayRanking({ wodLabel, wodMeta, rows, href, empty }) {
  return (
    <div className={s.rBox}>
      <div className={s.rTtl}>
        <IconTrophy className={s.rTtlIc} /> Ranking de hoje
      </div>
      {rows?.length ? (
        <>
          ...
          {rows.map((r, i) => (
            <div key={i} className={s.rk}>
              <span className={s.rkPos} style={{ '--pcol': `var(--podium-${i + 1})` }}>
                {i + 1}
              </span>
      ...
      {warnings.map(w => {
        ...
        return (
          <div key={w.id} className={s.warnCard}>
            <div className={s.warnTop}>
              <IconAlertTriangle className={s.warnIc} />
              <span className={s.warnDate}>{fmtWarnDate(w.date)}</span>
            </div>
