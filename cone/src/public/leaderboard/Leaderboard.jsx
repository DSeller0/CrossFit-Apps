    ...
    ;(daySessions || []).forEach(sess => {
      ;(sess.blocks || []).filter(bl => WOD_TYPES.includes(bl.label) || WOD_TYPES.includes(bl.type)).forEach(bl => {
        ...
        if (hasRes) {
          ...
          list.push({ key: `${dateKey}|${sess.id}|${bl.id}`, dateKey, sessId: sess.id, blId: bl.id, blType: bl.type, label, meta, sessName, dt })
        }
      })
    })
  })
...
export default function Leaderboard() {
  ...
  // Apply dynamic CSS variables from config/lbColors
  useEffect(() => {
    if (!appState) return
  ...
  // Apply URL deep-link params after data loads
  useEffect(() => {
    if (!appState) return
    ...
    if (wod && sessId && date) {
      ...
      if (list.find(w => w.key === key)) setSelWod(key)
    }
  }, [appState])

  // pageshow re-fetch (bfcache back-nav)
  useEffect(() => {
