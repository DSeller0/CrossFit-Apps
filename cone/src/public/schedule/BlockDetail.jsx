...
export default function BlockDetail({
  ...
  const athSection = onLogBlock && isWod && (
    <>
      <div
        className={`${styles.deskAthResultRow} ${athResult ? styles.deskAthResultRowLogged : styles.deskAthResultRowEmpty}`}
      >
        <span
          className={`${styles.deskAthResultName} ${athResult ? styles.deskAthResultNameLogged : styles.deskAthResultNameEmpty}`}
        >
          {athName || 'Atleta'}
        </span>
        {athResult ? (
      ...
      <button
        className={`${styles.deskRegBtn}${athResult ? ' ' + styles.deskRegBtnEdit : ''}`}
        onClick={e => {
          e.stopPropagation()
          onLogBlock()
        }}
  ...
  if (bl.type === 'Estações') {
    const stations = bl.stations || [],
      cycleCount = bl.stationRepeat || bl.rounds || 1
    const repeat = cycleCount > 1 ? `×${cycleCount}` : ''
    const capMins = stationsCapMins(bl)
    const stationHasEx = stations.some(
      st => !st.isRest && (st.exercises || []).some(e => e.name || e.isComplex),
    )
    return (
      <div className={styles.detailBlock} style={{ borderLeftColor: col }}>
        <div className={styles.detailBlockHdr}>
          <span className={styles.detailBlockTitle} style={{ color: col }}>
            {label}
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            >
              <i className="ti ti-player-play" /> Timer
            </button>
            {(repeat || capMins > 0) && (
              <span className={styles.detailBlockMeta}>
                {[repeat, capMins > 0 ? `Cap ${capMins}'` : ''].filter(Boolean).join(' · ')}
              </span>
            )}
        ...
        {stations.map((st, si) =>
          st.isRest ? (
            <div key={si} className={styles.detailStationRest}>
              {st.name || 'Descanso'}
            ...
            <div key={si} className={styles.detailStation}>
              ...
              {(st.exercises || [])
                ...
                .map((ex, ei) => (
                  <ExRow key={ei} ex={ex} {...sharedExProps} />
                ))}
            </div>
          ),
        )}
        {!stationHasEx &&
          (bl.exercises || [])
            .filter(e => e.name || e.isComplex)
            .map((ex, ei) => <ExRow key={ei} ex={ex} {...sharedExProps} />)}
        {bl.restBetweenCycles && (
          <div className={styles.detailBlockNotes}>
            Descanso entre ciclos: {bl.restBetweenCycles}
          </div>
        )}
  ...
  if (isRd) {
    ...
    if (allDone) rdBadgeEl = <span className={`${styles.rdProg} ${styles.rdProgComplete}`}>✓</span>
      ...
      rdBadgeEl = (
        <span className={`${styles.rdProg} ${styles.rdProgPartial}`}>
          RD {minDone} / {bl.rounds}
        </span>
      )
    else
      rdBadgeEl = <span className={`${styles.rdProg} ${styles.rdProgIdle}`}>{bl.rounds} RDS</span>
  }
  ...
  return (
    <div className={styles.detailBlock} style={{ borderLeftColor: col }}>
      <div className={styles.cardBody}>
        <div className={`${styles.cardInfo}${actionsEmpty ? ' ' + styles.cardInfoFull : ''}`}>
          {infoCol}
        </div>
        {!actionsEmpty && (
          <div className={styles.cardActions}>
            ...
            {isWod && (
              <a
                ...
                href={`leaderboard.html?wod=${bl.id}&session=${sess.id}&date=${dateKey}`}
                target="_blank"
            ...
            {athSection}
            {showIdleHint && (
              <div className={styles.deskIdleHint}>
                Selecione um atleta no painel ao lado para registrar resultados
              </div>
            )}
          </div>
        )}
