  ...
  if (ex.isComplex) {
    ...
    if (cxIsProg) {
      const steps = ex.intensity?.steps || [],
        unit = (steps[0]?.unit || '% RM').replace('% do RM', '% RM')
      ...
      if (exRm?.rm && pctNums.length)
        calcStr =
          pctNums.map(p => Math.ceil((exRm.rm * p) / 100)).join('/') + ' ' + (exRm.unit || 'kg')
    } else if (cxIsPct) {
      const pctNum = parseFloat(ex.intensity?.pct)
      if (ex.intensity?.pct) loadStr = ex.intensity.pct + '% RM'
      if (exRm?.rm && !isNaN(pctNum))
        calcStr = Math.ceil((exRm.rm * pctNum) / 100) + ' ' + (exRm.unit || 'kg')
    } else if (ins) {
      loadStr = ins
    }
        ...
        <div className={styles.detailExBody}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flex: 1, minWidth: 0 }}>
              ...
              <div
                className={`${styles.detailExName}${!isWod && done ? ' ' + styles.detailExNameDone : ''}`}
              >
                {displayName}
              </div>
            </div>
          ...
          {cxHasRm && rmEditKey === ex.id && (
            <div className={styles.rmInputWrap} onClick={e => e.stopPropagation()}>
              ...
              <button className={styles.rmConfirmBtn} onClick={confirmRm} aria-label="Confirmar RM">
                ✓
              </button>
            </div>
          )}
          {(loadStr || calcStr) && (
            <div className={styles.rmVolRow}>
              {loadStr && (
                <span className={loadStr.includes('%') ? styles.pillVol : styles.pillWt}>
                  {loadStr}
                </span>
              )}
              {calcStr && (
                <span className={exRm?.source === 'auto' ? styles.pillVol : styles.pillWt}>
                  {calcStr}
                </span>
              )}
            </div>
    ...
    return (
      <>
        {groups.map((g, gi) => {
          ...
          const calcStr =
            exRm?.rm && pctNums.length
              ? pctNums.map(p => Math.ceil((exRm.rm * p) / 100)).join('/') +
                ' ' +
                (exRm.unit || 'kg')
              : ''
              ...
              <div className={styles.detailExBody}>
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`${styles.detailExName}${!isWod && lineDone ? ' ' + styles.detailExNameDone : ''}`}
                    >
                      {ex.name}
                    </div>
                  </div>
    ...
    <div className={styles.detailEx} onClick={e => e.stopPropagation()}>
      ...
      <div className={styles.detailExBody}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0 }}>
            {ins && (
              <span className={ins.includes('%') ? styles.pillVol : styles.pillWt}>{ins}</span>
            )}
            <button
