      ...
      ;(sess.blocks || []).forEach(bl => {
        ;(bl.exercises || [])
          ...
          .forEach(ex => {
            const pr = prs.find(
              p =>
                p.type === 'load' && (p.name || '').toLowerCase() === (ex.name || '').toLowerCase(),
            )
            if (!pr) return
            const best = prBest(pr)
            if (!best?.value) return
            rm[ex.id] = {
              rm: Math.round(Number(best.value) * 10) / 10,
              unit: pr.unit || 'kg',
              source: 'auto',
            }
          })
...
export default function Schedule() {
  ...
  const [deskRegExerciseRows, setDeskRegExerciseRows] = useState(null)
  ...
  async function load(attempt = 0) {
    ...
    try {
      ...
      let restLbl = 'Descanso',
        gName = stD.gymName || 'Cone'
      if (cfgRes?.ok) {
        try {
      ...
      const pDate = sp.get('date'),
        pOpenLog = sp.get('openLog'),
        pBlockId = sp.get('blockId')
      ...
      if (pOpenLog && pDate) {
        ...
        if (sess) {
          const prefill = pBlockId
            ? {
                blockId: pBlockId,
                athId: pAthlete || '',
                perfTime: pPrefill || '',
                perfRounds: pPrefillRounds || '',
              }
            : null
  ...
  function isWodLogged(sess, bl) {
    ...
    return results.some(
      r =>
        r.sessionId === sess.id &&
        r.athleteId === selAth &&
        (r.blocks || []).some(b => b.blockId === bl.id),
    )
  }
  ...
  function getRd(blId, exId) {
    return roundState[`${blId}|${exId}`] || 0
  }

  function advanceRound(blId, exId, total) {
    setRoundState(prev => {
      const cur = prev[`${blId}|${exId}`] || 0,
        next = { ...prev, [`${blId}|${exId}`]: cur >= total ? 0 : cur + 1 }
      if (next[`${blId}|${exId}`] === 0) delete next[`${blId}|${exId}`]
      try {
        localStorage.setItem('sched_rounds', JSON.stringify(next))
      } catch {
        /* ignore */
      }
      return next
  ...
  function blockProgress(bl, sess) {
    ...
    if (isRoundBlock(bl)) {
      exs.forEach(ex => {
        if (!ex.isComplex && ex.intensity?.mode === 'progression') {
          ...
          if (getRd(bl.id, ex.id) >= Number(bl.rounds)) done++
        }
      })
    ...
    exs.forEach(ex => {
      if (!ex.isComplex && ex.intensity?.mode === 'progression') {
        ...
        if (checked.has(`${bl.id}|${ex.id}`)) done++
      }
    })
    return { done, total: exs.length }
  }
  ...
  function sessionProgress(sess) {
    const relevant = (sess.blocks || []).filter(
      bl => isWodBlock(bl) || (bl.exercises || []).some(e => e.name || e.isComplex),
    )
    const done = relevant.filter(bl => {
      const p = blockProgress(bl, sess)
  ...
  function changeWeek(dir) {
    ...
    setDeskRegBl(null)
    try {
      localStorage.removeItem('sched_rounds')
    } catch {
      /* ignore */
    }
  }
  ...
  function openTimer(bl, sess, dateKey) {
    ...
    let exercises,
      ...
      transitionTime = 15
    if (bl.type === 'Estações') {
      const nonRest = (bl.stations || []).filter(st => !st.isRest),
        rest = (bl.stations || []).filter(st => st.isRest)
      exercises = nonRest.map(st => ({
        name: st.name || 'Grupo',
    ...
    const config = {
      blockType: bl.type || bl.label,
      blockLabel:
        bl.label && bl.label !== bl.type && bl.label !== '-'
          ? bl.label
          : bl.label || bl.type || 'WOD',
      timeCap: bl.duration ? parseInt(bl.duration) : null,
      rounds: bl.rounds || bl.stationRepeat || null,
      exercises,
      sessionId: sess.id,
      sessionDate: dateKey,
  ...
  function doOpenLog(sess, dateKey, aths, athId, prefill = null, onlyBlockId = null) {
    const targets = Array.isArray(sess.mainTraining)
      ? sess.mainTraining
      : sess.mainTraining
        ? [sess.mainTraining]
        : []
    ...
    if (resolvedAthId && !candidates.some(a => String(a.id) === String(resolvedAthId)))
      resolvedAthId = ''
    ...
    const existing = resolvedAthId
      ? results.find(r => r.sessionId === sess.id && r.athleteId === resolvedAthId)
      : null
    ...
    if (prefill?.blockId) {
      ...
      if (bi >= 0) {
        if (prefill.perfTime) blocks[bi].perfTime = prefill.perfTime
        if (prefill.perfRounds) blocks[bi].perfRounds = prefill.perfRounds
      }
    }
    setLogPane({ sess, dateKey, assignedAth: candidates })
    setLogAthId(resolvedAthId)
    setLogBlocks(blocks)
  ...
  function changeLogAthId(newAthId) {
    ...
    if (!logPane) return
    const existing = newAthId
      ? results.find(r => r.sessionId === logPane.sess.id && r.athleteId === newAthId)
      : null
    setLogBlocks(prev =>
      prev.map(b => {
  ...
  function setLogConfirmStep(on) {
    if (on && !logAthId) {
      setLogError('Selecione seu nome antes de enviar.')
      return
    }
    setLogError('')
    setLogConfirming(on)
  ...
  async function submitLog() {
    ...
    const result = {
      ...
      athleteId: logAthId,
      sessionId: sess.id,
      presence: existing?.presence || 'Presente',
      energyLevel: existing?.energyLevel ?? 3,
      blocks: mergedBlocks,
      coachNote: existing?.coachNote || '',
      flagForReview: existing?.flagForReview || false,
      loggedByAthlete: true,
    }
    const next = [
      ...existingArr.filter(r => !(r.athleteId === logAthId && r.sessionId === sess.id)),
      result,
    ]
    const { error } = await sb.rpc('log_result', {
      p_id: String(result.id),
      p_date: result.date,
      p_athlete_id: result.athleteId,
      p_session_id: result.sessionId ? String(result.sessionId) : null,
      p_presence: result.presence,
      p_energy_level: result.energyLevel ?? null,
      p_blocks: result.blocks,
    })
    if (error) {
      setLogSubmitting(false)
      setLogError('Erro ao enviar. Tente novamente.')
      return
    }
    setResults(next)
    setLogSubmitting(false)
  ...
  function deskOpenReg(bl, sess, dateKey) {
    ...
    setDeskRegExerciseRows(existingBlock?.exerciseRows ?? null)
    setDeskRegError('')
  }
  ...
  async function submitDeskReg() {
    ...
    if (!selAth || !deskRegBl) {
      setDeskRegError('Selecione um atleta primeiro.')
      return
    }
    setDeskRegSubmitting(true)
    setDeskRegError('')
    ...
    if (error) {
      setDeskRegSubmitting(false)
      setDeskRegError('Erro ao enviar. Tente novamente.')
      return
    }
    setResults(prev => [
      ...prev.filter(r => !(r.athleteId === selAth && r.sessionId === sess.id)),
      result,
    ])
    setDeskRegSubmitting(false)
    setDeskRegStep('success')
  }
  ...
  const selSessDateStr = selSess
    ? new Date(selSess.dateKey + 'T12:00:00').toLocaleDateString('pt-BR', {
        weekday: 'short',
        day: '2-digit',
        month: '2-digit',
      })
  ...
  return (
    <>
      ...
      <LogPane
        pane={logPane}
        athId={logAthId}
        onAthId={changeLogAthId}
        blocks={logBlocks}
        onBlocks={setLogBlocks}
        submitting={logSubmitting}
        success={logSuccess}
        error={logError}
        confirming={logConfirming}
        onConfirming={setLogConfirmStep}
        onSubmit={submitLog}
        onClose={() => {
          setLogPane(null)
          setLogSuccess(false)
        ...
        lockedAthName={
          lockedId ? athletes.find(a => String(a.id) === String(lockedId))?.name || '' : ''
        }
      />
                ...
                <div className={styles.weekGrid}>
                  {week.map(date => {
                    const dk = toISO(date),
                      isPast = dk < today,
                      isToday = dk === today
                    const daySess = sessionsForDay(dk),
                      hasSess = daySess.length > 0
                    return (
                      <div
                        key={dk}
                        className={`${styles.dayCard}${isPast ? ' ' + styles.past : ''}${isToday ? ' ' + styles.today : ''}${hasSess ? ' ' + styles.hasSess : ''}`}
                      >
                          ...
                          {hasSess ? (
                            daySess.map((sess, si) => {
                              const prog = sessionProgress(sess),
                                isExp = expanded.has(`${dk}|${si}`)
                              const blocks = sess.blocks || []
                              const exNames = [
                                ...new Set(
                                  blocks.flatMap(bl =>
                                    (bl.exercises || []).filter(e => e.name).map(e => e.name),
                                  ),
                                ),
                              ].slice(0, 3)
                              const moreEx =
                                blocks.flatMap(b => b.exercises || []).filter(e => e.name).length >
                                3
                              const sessResult =
                                isExp && selAth
                                  ? results.find(
                                      r => r.sessionId === sess.id && r.athleteId === selAth,
                                    )
                                  : null
                              return (
                                <div key={sess.id}>
                                  ...
                                  >
                                    {sess.sessionName && (
                                      <div className={styles.sessName}>{sess.sessionName}</div>
                                    )}
                                    {sess.mainTraining && !lockedId && (
                                      <div className={styles.sessAlvo}>
                                        {Array.isArray(sess.mainTraining)
                                    ...
                                    <div className={styles.blockBadges}>
                                      {blocks.map(bl => {
                                        const lbl = blkLabel(bl),
                                          p = blockProgress(bl, sess),
                                          blDone = p.total > 0 && p.done === p.total
                                        const bc = blkColor(bl)
                                        return (
                                          <span
                                            key={bl.id}
                                  ...
                                  {isExp && (
                                    <SessionDetail
                                      ...
                                      onReset={resetRound}
                                      onRmToggle={key =>
                                        setRmEditKey(k => (k === key ? null : key))
                                      }
                                      onRmConfirm={(exId, rm, unit) => {
                                        setRmValues(prev => ({
                                          ...prev,
                                          [exId]: { rm, unit, source: 'manual' },
                                        }))
                                        setRmEditKey(null)
                                      }}
                                      ...
                                      onLogBlock={
                                        selAth
                                          ? bl => doOpenLog(sess, dk, athletes, selAth, null, bl.id)
                                          : null
                                      }
                                      getAthResult={bl =>
                                        sessResult?.blocks?.find(b => b.blockId === bl.id) || null
                                      }
                            ...
                            <div className={styles.restLabel}>
                              <i className="ti ti-moon" /> {restLabel}
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className={styles.deskStrip}>
                  {week.map(date => {
                    const dk = toISO(date),
                      isPast = dk < today,
                      isToday = dk === today
                    const daySess = (sessions[dk] || []).filter(
                      s => s.public !== false && inBoxScope(s, box) && s.blocks && s.blocks.length,
                    )
                    return (
                      <div key={dk} className={styles.deskDayCol}>
                        <div
                          className={`${styles.deskDayHdr}${isToday ? ' ' + styles.deskDayHdrToday : ''}`}
                        >
                          <span className={styles.deskDow}>{DAY_PT[date.getDay()]}</span>
                          <span
                            className={`${styles.deskDnum}${isPast ? ' ' + styles.deskDnumPast : ''}`}
                          >
                            {date.getDate()}
                          </span>
                        </div>
                          ...
                          daySess.map(sess => {
                            ...
                            return (
                              <div
                                ...
                                aria-pressed={isSel}
                                onClick={() => {
                                  setSelSess(isSel ? null : { dateKey: dk, sessId: sess.id })
                                  setDeskRegBl(null)
                                }}
                                onKeyDown={onKey(() => {
                                  setSelSess(isSel ? null : { dateKey: dk, sessId: sess.id })
                ...
                <div className={styles.deskBody}>
                    ...
                    {lockedId ? (
                      ...
                      <>
                        <div className={styles.deskAthSearchWrap}>
                          ...
                          <input
                            className={styles.deskAthSearchInput}
                            type="text"
                            placeholder="Buscar…"
                            value={deskAthSearch}
                            onChange={e => setDeskAthSearch(e.target.value)}
                          />
                        </div>
                        <div className={styles.deskAthList}>
                          {athletes
                            .filter(
                              a =>
                                !deskAthSearch ||
                                a.name.toLowerCase().includes(deskAthSearch.toLowerCase()),
                            )
                            ...
                            .map(ath => {
                              ...
                              return (
                                ...
                                >
                                  <span
                                    className={`${styles.deskAthDot}${hasLogged ? ' ' + styles.deskAthDotFilled : ''}`}
                                  />
                                  {ath.name}
                                </div>
                              )
                    ...
                    {selSessObj ? (
                      <>
                        <div className={styles.deskSessPaneHdr}>
                          ...
                          <span className={styles.deskSessNameHdr}>
                            {sessName(selSessObj, selSess.dateKey)}
                          </span>
                        ...
                        <div className={styles.deskSessScroll}>
                          {(selSessObj.blocks || []).map(bl => {
                            ...
                            const existingResult = selAth
                              ? results
                                  .find(
                                    r => r.sessionId === selSessObj.id && r.athleteId === selAth,
                                  )
                                  ?.blocks?.find(b => b.blockId === bl.id) || null
                              : null
                            return (
                              <BlockDetail
                                key={bl.id}
                                ...
                                onLogBlock={
                                  isWod && selAth
                                    ? () => deskOpenReg(bl, selSessObj, selSess.dateKey)
                                    : null
                                }
                  ...
                  {deskRegBl && (
                    <DeskRegPane
                      ...
                      onExerciseRows={setDeskRegExerciseRows}
      ...
      {checkinId && (
        <CheckinSheet
          checkinExec={checkinExec}
          checkinDone={checkinDone}
          checkinMode={checkinMode}
          onCheckinMode={setCheckinMode}
          checkinSearch={checkinSearch}
          onCheckinSearch={setCheckinSearch}
          athletes={athletes}
          checkinAthId={checkinAthId}
          onCheckinAthId={setCheckinAthId}
