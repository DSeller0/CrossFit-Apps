...
export function prBest(pr) {
  ...
  if (pr.type === 'time')
    return pr.results.reduce((b, r) => (toSecs(r.value) < toSecs(b.value) ? r : b))
...
export function prDelta(pr) {
  ...
  if (diff === 0) return { label: '=', good: null }
  return {
    label: (diff > 0 ? '+' : '') + diff + ' ' + (pr.type === 'load' ? pr.unit || 'kg' : 'reps'),
    good: diff > 0,
  }
}
...
export function prPct(pr) {
  ...
  if (pr.type === 'time') {
    ...
    return Math.min(
      100,
      Math.round(((firstSecs - toSecs(best.value)) / (firstSecs - targetSecs)) * 100),
    )
  }
