...
describe('prBest', () => {
  ...
  test('type time: lowest value wins', () => {
    const pr = { type: 'time', results: [{ value: '05:30' }, { value: '04:45' }, { value: '06:00' }] }
...
describe('prPct', () => {
  ...
  test('type time: progress toward target from first result', () => {
    const pr = { type: 'time', target: '10:00', results: [
...
describe('prDelta', () => {
  test('fewer than 2 results → null', () => expect(prDelta({ results: [{ value: '80' }] })).toBe(null))
  test('type load: improvement', () => {
    const pr = { type: 'load', results: [
