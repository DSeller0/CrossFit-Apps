...
import { DEF_INP, inputKey, sessName, blockEntries, calcKpis, cardSummary } from './resultsHelpers.js'
...
function pickDefaultWod(week, sessions, lockedAthName, today, box) {
  ...
  if (days.includes(today)) { const w = firstWodOn(today); if (w) return w }
  for (const dk of days.filter(d => d < today).reverse()) { const w = firstWodOn(dk); if (w) return w }
  for (const dk of days) { const w = firstWodOn(dk); if (w) return w }
  return null
}
...
export default function Results() {
    ...
    try {
      ...
      if (urlSid) {
        for (const [dk, list] of Object.entries(sD)) {
          ...
          const wods = wodBlocks(sess)
          if (wods.length) { setSelWod({ sid: sess.id, bid: wods[0].id, dk }); didAutoSelect.current = true }
          break
        }
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (urlSid && expanded.has(urlSid)) {
      ...
      requestAnimationFrame(() => {
        document.querySelector(`[data-card-id="${urlSid}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
      })
    }
  }, [expanded, status])
  ...
  function cancelEdit(sid, bid) {
    const k = inputKey(sid, bid)
    setEditing(prev => { const n = new Set(prev); n.delete(k); return n })
    setLogInputs(prev => { const n = { ...prev }; delete n[k]; return n })
  }
  function isEditing(sid, bid) { return editing.has(inputKey(sid, bid)) }
  ...
  async function doSubmit(sid, bid, dk) {
    ...
    const blockEntry = {
      blockId: bid, blockType: btype, blockLabel: lbl, rpe: inp.rpe, scale: inp.scale,
    ...
    const { error: e } = await sb.rpc('log_result', {
      p_id: String(newResult.id), p_date: newResult.date, p_athlete_id: newResult.athleteId,
      p_session_id: newResult.sessionId ? String(newResult.sessionId) : null,
      p_presence: newResult.presence, p_energy_level: newResult.energyLevel ?? null, p_blocks: newResult.blocks,
    })
  ...
  const selEntries  = (selWod && selBlock) ? blockEntries(results, athletes, selWod.sid, selWod.bid) : []
  ...
  function wodBody(sess, bl, dk, variant) {
    ...
    return (
      <>
        ...
        {!selAth && <KpiGrid kpis={kpis} btype={bl.type} variant={variant === 'extended' ? 'extended' : 'compact'} />}
        {selAth && br && !edit && (
          <LoggedResult br={br} btype={bl.type} onEdit={() => startEdit(sess.id, bl.id)} />
        )}
        {selAth && (!br || edit) && (
          <LogForm bl={bl} inp={getInp(sess.id, bl.id)} isSubmitting={submittingKey === k}
    ...
    <>
      ...
      <div className={`${styles.lbPane}${lbTarget ? ' ' + styles.lbPaneOpen : ''}`}>
        <div className={styles.lbPaneHdr}>
          <div className={styles.lbPaneTitle}><i className="ti ti-trophy" aria-hidden="true" /> {lbTitle}</div>
          <button className={styles.lbPaneClose} onClick={() => setLbTarget(null)} aria-label="Fechar">
      ...
      <div className={`${styles.confirmModal}${confirmPending ? ' ' + styles.confirmModalOpen : ''}`} role="dialog" aria-modal="true">{confirmContent}</div>
        ...
        <main className={styles.main}>
          ...
          {status !== 'loading' && <>
            {!lockedId && <div className={`${styles.selBar} ${styles.mobileOnly}`}>
              <label className={styles.srOnly} htmlFor="ath-sel">Filtrar por atleta</label>
              <select id="ath-sel" className={styles.athleteSel} value={selAth} onChange={e => changeAth(e.target.value)}>
                <option value="">— Todos —</option>
                {athletes.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
              </select>
            </div>}
            <div className={styles.weekNav}>
              <button className={styles.navBtn} onClick={() => changeWeek(-1)} aria-label="Semana anterior"><i className="ti ti-chevron-left" aria-hidden="true" /></button>
              <span className={styles.weekLabel}>{weekLabel}</span>
              <button className={styles.navBtn} onClick={() => changeWeek(1)} aria-label="Próxima semana"><i className="ti ti-chevron-right" aria-hidden="true" /></button>
            </div>
          </>}

          {status === 'loading' && <div className={styles.loading} aria-live="polite"><i className={`ti ti-loader ${styles.spin}`} style={{ fontSize: '32px' }} aria-hidden="true" />Carregando...</div>}
          {status === 'error' && <div className={styles.errorMsg} aria-live="polite"><i className="ti ti-alert-circle" style={{ fontSize: '32px' }} aria-hidden="true" /><br /><br />Não foi possível carregar os dados.<br /><small>{errMsg}</small><br /><button className={styles.retryBtn} onClick={() => load()}>↺ Tentar novamente</button></div>}
                  ...
                  return (
                    <div key={dk} className={`${styles.calDay}${isToday ? ' ' + styles.calDayToday : ''}`}>
                      ...
                      {hasWods
                            ...
                            return (
                              <div key={`${sess.id}:${bl.id}`}
                                ...
                                <div className={styles.calCardRow}>
                                  <span className={`${styles.calDot}${cnt > 0 ? ' ' + styles.calDotFill : ''}`} />
                        ...
                        : <div className={styles.calRest}><i className="ti ti-moon" aria-hidden="true" /></div>
                      }
                    </div>
                  )
                })}
                  ...
                  {lockedId
                    ...
                    : <>
                        <div className={styles.athSearchWrap}>
                          ...
                          <input className={styles.athSearchInput} type="text" placeholder="Buscar..." aria-label="Buscar atleta"
                        ...
                        <div className={styles.athList}>
                          <div className={`${styles.athRow} ${styles.athRowTodos}${!selAth ? ' ' + styles.athRowSel : ''}`}
                            role="button" tabIndex={0} aria-pressed={!selAth}
                            onClick={() => changeAth('')} onKeyDown={onKey(() => changeAth(''))}>◈ Todos</div>
                          {filteredAthletes.map(a => {
                            const isSel = selAth === String(a.id)
                            return (
                              <div key={a.id} className={`${styles.athRow}${isSel ? ' ' + styles.athRowSel : ''}`}
                                role="button" tabIndex={0} aria-pressed={isSel}
                                onClick={() => changeAth(String(a.id))} onKeyDown={onKey(() => changeAth(String(a.id)))}>
                                {a.name}
                              </div>
                            )
                          })}
                ...
                <div className={styles.midPane}>
                  {selWod && selBlock && selSess
                        ...
                        <div className={styles.midScroll}>
                          {selAth && (
                            <div className={styles.exListLbl}>
                              {selAthBlock && !isEditing(selWod.sid, selWod.bid) ? 'Resultado' : 'Registrar'} · {selAthName}
                            </div>
                          )}
                          {wodBody(selSess, selBlock, selWod.dk, 'extended')}

                          {/* Where the athlete stands, once they have a result. */}
                          {selAth && selAthBlock && !isEditing(selWod.sid, selWod.bid) && selAthRank && (
                            <div className={styles.relKpiRow}>
                              <div className={styles.relKpi}>
                                <div className={styles.relKpiVal}>{MEDALS[selAthRank - 1] || `${selAthRank}º`} / {selEntries.length}</div>
                              ...
                              {selEntries.length > 0 && (
                                <div className={styles.relKpi}>
                                  <div className={styles.relKpiVal}>Top {Math.round(selAthRank / selEntries.length * 100)}%</div>
                    ...
                    : <div className={styles.paneEmpty}>
                        <i className="ti ti-layout-grid" style={{ fontSize: '26px', opacity: .3 }} aria-hidden="true" />
                  ...
                  {selWod && selBlock
                    ? <>
                        ...
                        <div className={styles.deskLbList}>
                          <RankList entries={selEntries} blType={selBlock.type}
                            scaleFilter={lbScale} highlightAthleteId={selAth} />
                        </div>
                      </>
                    : <div className={styles.paneEmpty}>
                        <i className="ti ti-trophy" style={{ fontSize: '26px', opacity: .3 }} aria-hidden="true" />
                        Nenhum WOD nesta semana
                      </div>
                  }
                </div>
                  ...
                  <div key={dk} className={`${styles.dayCard}${isToday ? ' ' + styles.dayCardToday : ''}`}>
                    ...
                    <div className={styles.dayBody}>
                      {daySess.map(sess => {
                        ...
                        const entries = firstWod ? blockEntries(results, athletes, sess.id, firstWod.id) : []
                            ...
                            {wods.length
                              ? wods.map(bl => (
                                  <div key={bl.id} className={styles.wodSection}>
                                    ...
                                    <button className={styles.btnLb} onClick={() => setLbTarget({ sid: sess.id, bid: bl.id, dk })}>
